...
class Admin extends BaseController
{
  ...
  public function section($action = "", $id = "")
  {
    ...
    if ($action == 'list') {
      ...
      return view('backend/admin/section/list', $page_data);
    }
  }
  //	SECTION ENDED
  ...
  public function class_room($param1 = '', $param2 = '', $param3 = '')
    {
      ...
      echo json_encode(array('status' => $response, 'csrf' => $csrf));
       ...
       if ($param1 == 'list') {
          return view('backend/admin/class_room/list', ['classe_id' => '', 'room_id' => '']);
        }
  ...
  public function subject($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'list') {
      ...
      return view('backend/admin/subject/list', $page_data);
    }
  ...
  public function class_wise_subject($class_id)
  {
    ...
    return view('backend/admin/subject/dropdown', $page_data);
  }
  //END SUBJECT section
  ...
  {
    $allowed_user_types = ['admin', 'teacher', 'superadmin'];
    ...
    if ($param1 == 'list') {
      ...
      return view('backend/admin/syllabus/list', $page_data);
    }
  ...
  {
    $allowed_user_types = ['admin', 'teacher', 'student', 'superadmin'];
    $current_user_type = session()->get('user_type');
    $current_user_id = session()->get('user_id');
  ...
  public function school_crud($param1 = "", $param2 = "", $param3 = "")
  {
    ...
    if ($param1 == 'list') {
      ...
      return view('backend/admin/school/list');
    }
  ...
  public function teacher($param1 = '', $param2 = '', $param3 = '')
  {
    ...
    if ($param1 == 'status') {
      $response = $this->user_model->update_teacher_status($param2, $param3);
      $csrf = array(
        'csrfName' => csrf_token(),
        'csrfHash' => csrf_hash(),
      );
    ...
    if ($param1 == 'list') {
      return view('backend/admin/teacher/list');
    }
  ...
  public function permission($param1 = '', $param2 = '', $param3 = '')
  {
    ...
    if ($param1 == 'filter') {
      ...
      $page_data['class_id'] = $class_id;
      return view('backend/admin/permission/list', $page_data);
    }
    ...
    $page_data['page_title'] = 'permission_history';
    ...
    if (empty($param1)) {
      ...
      $firstClass = $this->db->table('classes')
          ->select('id')
          ->where('school_id', $school_id)
          ->orderBy('id', 'ASC')
          ->limit(1)
      ...
      if ($firstClass) {
          $page_data['class_id'] = $firstClass->id;
      } else {
  ...
  public function get_permission_history()
  {
      if (session()->get('admin_login') != 1) {
          log_message('warning', 'Unauthorized access attempt to get_permission_history from IP: ' . $this->request->getIPAddress());
          echo json_encode(['status' => false, 'message' => 'Unauthorized']);
          exit;
      }

      $rateLimitKey = 'permission_history_rate_limit_' . session()->get('user_id');
      ...
      if ($rateLimitCount >= 30) {
          log_message('warning', 'Rate limit exceeded for get_permission_history by user_id: ' . session()->get('user_id'));
          echo json_encode(['status' => false, 'message' => 'Too many requests. Please try again later.']);
          exit;
      }
      ...
      if ($teacher_id <= 0 || $class_id <= 0) {
          log_message('warning', 'Invalid request to get_permission_history by user_id: ' . 
              session()->get('user_id') . ', teacher_id: ' . $teacher_id . ', class_id: ' . $class_id);
      ...
      if (!$teacher) {
          log_message('warning', 'Unauthorized permission history view attempt by user_id: ' . 
              session()->get('user_id') . ' for teacher_id: ' . $teacher_id . ', school_id: ' . school_id());
          echo json_encode(['status' => false, 'message' => 'Unauthorized']);
          exit;
      }
      ...
      if (!$class) {
          log_message('warning', 'Unauthorized permission history view attempt by user_id: ' . 
              session()->get('user_id') . ' for class_id: ' . $class_id . ', school_id: ' . school_id());
          echo json_encode(['status' => false, 'message' => 'Unauthorized']);
          exit;
      }

      log_message('info', 'Permission history viewed by user_id: ' . session()->get('user_id') . 
          ' for teacher_id: ' . $teacher_id . ', class_id: ' . $class_id);
      
      $builder = $this->db->table('permission_history ph');
      $builder->select('ph.*, u_teacher.name as teacher_name, c.name as class_name, u_changed.name as changed_by_name');
      $builder->join('teachers t', 'ph.teacher_id = t.id');
      $builder->join('users u_teacher', 't.user_id = u_teacher.id');
      $builder->join('classes c', 'ph.class_id = c.id');
      $builder->join('users u_changed', 'ph.changed_by = u_changed.id');
      $builder->where('ph.teacher_id', $teacher_id);
      $builder->where('ph.class_id', $class_id);
      $builder->where('ph.school_id', school_id());
      $builder->orderBy('ph.created_at', 'DESC');
      $builder->limit(50);
  ...
  //END TEACHER PERMISSION section
    ...
    public function Liveclasse($param1 = '', $param2 = '', $param3 = '')
    {
      ...
      echo json_encode(array('status' => $response, 'csrf' => $csrf));
       ...
       if ($param1 == 'list') {
          return view('backend/admin/bigbleubutton/list');
        }
			...
			public function delete_recording()
{
  ...
  public function accountant($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'list') {
      return view('backend/admin/accountant/list');
    }
  ...
  public function librarian($param1 = '', $param2 = '')
  {
    ...
    // show data from database
    if ($param1 == 'list') {
      return view('backend/admin/librarian/list');
    }
  ...
  public function routine($param1 = '', $param2 = '', $param3 = '', $param4 = '')
  {
    ...
    if ($param1 == 'filter') {
      ...
      return view('backend/admin/routine/list', $page_data);
    }
  ...
  public function attendance($param1 = '', $param2 = '', $param3 = '')
  {
    ...
    if ($param1 == 'filter') {
      ...
      $page_data['year'] = htmlspecialchars($this->request->getPost('year'));
        ...
        $response_html = view('backend/admin/attendance/list', $page_data, ['cache' => 0]);
    ...
    if ($param1 == 'student') {
      ...
      // Charger la vue mise Ã  jour
      $response_html = view('backend/admin/attendance/student', $page_data, ['cache' => 0]);
         // PrÃ©parer le nouveau jeton CSRF
         $csrf = array(
          'csrfName' => csrf_token(),
          'csrfHash' => csrf_hash(),
  ...
  public function event_calendar($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'list') {
      return view('backend/admin/event_calendar/list');
    }
  ...
  {
    // Keep authenticated/session context; only clear legacy filter keys if they exist.
   ...
   if ($param1 == 'edit') {
    ...
    if ($param1 == 'filter') {
            $page_data['class_id'] = ($param2 == '' || $param2 == 'all' || $param2 == 'undefined' || $param2 == 'null') ? 'all' : $param2;
            $page_data['section_id'] = ($param3 == '' || $param3 == 'all' || $param3 == 'undefined' || $param3 == 'null') ? 'all' : $param3;
            $page_data = $this->prepareStudentListDataForView($page_data);
            $html_content = view('backend/admin/student/list', $page_data);
            $csrf = array(
                'csrfName' => csrf_token(),
                'csrfHash' => csrf_hash(),
            );
  ...
  private function prepareStudentListDataForView(array $page_data): array
  {
    ...
    if ($school_id <= 0) {
      $school_id = (int) session()->get('active_school_id');
    }
    if ($school_id <= 0) {
    ...
    $school_name = '';
    if ($school_id > 0) {
      $school_name = (string) ($this->db->table('schools')->where('id', $school_id)->get()->getRow()->name ?? '');
    }
    ...
    $whereBase = [];
    if ($school_id > 0) {
      $whereBase['school_id'] = $school_id;
    }
    if (!empty($class_id) && $class_id !== 'all') {
      $whereBase['class_id'] = $class_id;
    }

    $activeSession = active_session();
    $enrolBuilder = $this->db->table('enrols')->select('DISTINCT(student_id), school_id, session');
    if (!empty($whereBase)) {
      $enrolBuilder->where($whereBase);
    }
    if (!empty($activeSession)) {
      $enrolBuilder->where('session', $activeSession);
    }
    $enrols = $enrolBuilder->get()->getResultArray();

    // Legacy/migrated fallback: some enrols rows may not be bound to current session.
    if (empty($enrols) && !empty($activeSession)) {
      $fallbackBuilder = $this->db->table('enrols')->select('DISTINCT(student_id), school_id, session');
      if (!empty($whereBase)) {
        $fallbackBuilder->where($whereBase);
      }
      $enrols = $fallbackBuilder->get()->getResultArray();
    }
    $rows = [];
    $active_count = 0;

    foreach ($enrols as $enrol) {
      $student = $this->db->table('students')->where('id', $enrol['student_id'])->get()->getRowArray();
      if (empty($student)) {
        continue;
      }
      $user = $this->db->table('users')->where('id', $student['user_id'])->get()->getRowArray();
      if (empty($user)) {
        continue;
      }
  ...
  {
    return $this->db->table('classes')->where('school_id', school_id())->get()->getResultArray();
  }
  ...
  public function exam($param1 = '', $param2 = '')
{
    if ($param1 == 'create') {
        ...
        $output = array(
            ...
            'message' => $response['notification'] ?? 'Exam created',
    ...
    if ($param1 == 'update') {
        ...
        if ($exam) {
            $exam['formatted_date'] = date('D, d-M-Y H:i', strtotime($exam['starting_date']));
    ...
    if ($param1 == 'delete') {
        ...
        if (is_string($response)) {
            ...
            return $this->response->setJSON($decoded ?? ['status' => false, 'message' => $response]);
        }
        return $this->response->setJSON($response);
    }
    ...
    if ($param1 == 'list') {
        ...
        if (!empty($page_data['filter_date_range']) && $date_from && $date_to) {
            $countBuilder->where('starting_date >=', date('Y-m-d H:i:s', $date_from));
            $countBuilder->where('starting_date <=', date('Y-m-d H:i:s', $date_to));
        }
        ...
        if (!empty($page_data['filter_date_range']) && $date_from && $date_to) {
            $examBuilder->where('exams.starting_date >=', date('Y-m-d H:i:s', $date_from));
            $examBuilder->where('exams.starting_date <=', date('Y-m-d H:i:s', $date_to));
        }
        ...
        foreach ($exams as $exam) {
            $exam_calendar[] = [
                'id' => $exam['id'],
                'title' => $exam['name'],
                'start' => date('Y-m-d H:i:s', strtotime($exam['starting_date'])),
            ];
        }
        $page_data['exam_calendar_json'] = json_encode($exam_calendar);
        return view('backend/admin/exam/list', $page_data);
    }
  ...
  // Paginated exams for AJAX loading (admin)
  public function get_exams_paginated()
  {
    ...
    $countBuilder = $this->db->table('exams')->where('school_id', $school_id);
    if ($class_id !== '') {
      $countBuilder->where('class_id', (int) $class_id);
    }
    if ($date_from && $date_to) {
    ...
    $total_exams = (int) $countBuilder->countAllResults();
    $total_pages = max(1, (int) ceil($total_exams / $limit));
    ...
    $examBuilder->select('exams.*, classes.name as class_name')
      ...
      ->where('exams.school_id', $school_id);
    if ($class_id !== '') {
      $examBuilder->where('exams.class_id', (int) $class_id);
    }
    if ($date_from && $date_to) {
    ...
    $exams = $examBuilder->orderBy('exams.id', 'DESC')->limit($limit, $offset)->get()->getResultArray();

    $exam_data = [];
    foreach ($exams as $exam) {
      $startingTs = (int) ($exam['starting_date'] ?? 0);
      $exam_data[] = [
        'id' => (int) ($exam['id'] ?? 0),
        'name' => (string) ($exam['name'] ?? 'Unnamed Exam'),
        'formatted_date' => $startingTs > 0 ? date('D, d-M-Y H:i', $startingTs) : 'No Date',
        'class_name' => (string) ($exam['class_name'] ?? 'No Class'),
      ];
    }
  ...
  public function mark($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'list') {
      ...
      // Charger la vue et capturer le contenu
        $html_content = view('backend/admin/mark/list', $page_data, ['cache' => 0]);
   ...
   public function quiz_result($param1 = '', $param2 = '')
   {
     ...
     if ($param1 == 'list') {
       ...
       $page_data['quiz_id'] = htmlspecialchars($this->request->getPost('quiz_id'));
            // Charger la vue et capturer le contenu
            $html_content = view('backend/admin/quiz/list', $page_data, ['cache' => 0]);
   ...
   public function quiz($action = "", $id = "")
   {
     ...
     if ($action == 'list') {
       $page_data['class_id'] = $id;
       return view('backend/admin/quiz/list_quiz', $page_data);
     }
   }
  ...
  public function grade($param1 = "", $param2 = "")
  {
    ...
    if ($param1 == 'update') {
      ...
      return $this->response->setJSON(array('status' => $response['status'] ?? $response, 'csrf' => $csrf));
    }
    ...
    // show data from database
    if ($param1 == 'list') {
      return view('backend/admin/grade/list');
    }
  ...
  function promotion($param1 = "", $promotion_data = "")
  {
    ...
    if ($param1 == 'list') {
      ...
      $page_data['class_from_details'] = $this->crud_model->get_classes($this->request->getPost('class_id_from'));
      $page_data['class_to_details'] = $this->crud_model->get_classes($this->request->getPost('class_id_to'));
      $page_data['session_from_details'] = $this->crud_model->get_session($this->request->getPost('session_from'));
      $page_data['session_to_details'] = $this->crud_model->get_session($this->request->getPost('session_to'));
      $page_data['enrolments'] = $this->crud_model->get_student_list();

        // Charger la vue et capturer le contenu
        $html_content = view('backend/admin/promotion/list', $page_data, ['cache' => 0]);
  ...
  public function invoice($param1 = "", $param2 = "")
  {
    ...
    if ($param1 == 'student') {
      ...
      return view('backend/admin/student/dropdown', $page_data);
    }
    ...
    if ($param1 == 'list') {
      ...
      return view('backend/admin/invoice/list', $page_data);
    }
    // showing the index file
    if (empty($param1)) {
      $page_data['folder_name'] = 'invoice';
  ...
  public function export($param1 = "", $date_from = "", $date_to = "", $selected_class = "", $selected_status = "")
  {
    ...
    if ($param1 == 'pdf' || $param1 == 'print') {
      ...
      ob_start();
      return view('backend/admin/invoice/export', $page_data);
      $html = ob_get_clean();
  ...
  public function payment($param1 = "", $invoice_id = "") {
    $is_superadmin = session()->get('user_type') === 'superadmin';
    $is_admin = session()->get('admin_login') == 1;
    ...
    if (empty($invoice_id) || !is_numeric($invoice_id)) {
      throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound('Invalid invoice ID');
      return;
    }
    ...
    log_message('error', "Payment - user_type: " . ($current_user_type ?? 'null') . ", current_school_id: " . ($current_school_id ?? 'null') . ", invoice_school_id: " . ($invoice_school_id ?? 'null'));

    if (empty($current_user_type)) {
      log_message('error', "Unauthenticated access attempt to invoice #{$invoice_id}");
      throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound('Please login first');
      return;
    }

    if ($current_user_type !== 'superadmin' && $current_user_type !== 'admin' && $invoice_school_id != $current_school_id) {
      log_message('error', "Unauthorized access attempt to invoice #{$invoice_id} by user type: {$current_user_type}");
      throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound('Access Denied');
      return;
    }

    $school = $this->db->table('schools')->where('id', $page_data['invoice_details']['school_id'])->get()->getRowArray();
    ...
    $page_data['prix_affiche_mad'] = $base_amount;
    $page_data['prix_affiche'] = $prix_affiche;
    ...
    $page_data['user_details'] = $this->db->table('users')->where('id', $student_id)->get()->getRowArray();
    ...
    $page_data['school'] = $this->db->table('schools')->where('id', $school_id)->get()->getRow();

    $page_data['amount_to_pay'] = $prix_affiche;
    $page_data['currency'] = $devise_affiche;
    $page_data['stripe_amount'] = $converted_amount;
    $page_data['stripe_currency'] = $payment_currency;
    $page_data['conversion_info'] = $conversion_info;
    $page_data['prix_base_mad'] = $base_amount;
    ...
    $paypal_rate = $this->get_conversion_rate($base_currency, 'USD');
    $paypal_converted_amount = round($base_amount * $paypal_rate, 2);

    $page_data['paypal_fx_rate_val'] = $paypal_rate;
    $page_data['paypal_sub_total_converted'] = round($base_amount * $paypal_rate, 2);
    $page_data['paypal_vat_amount_converted'] = round(($base_amount * $vat_rate / 100) * $paypal_rate, 2);
    $page_data['paypal_converted_amount'] = $paypal_converted_amount;

    $community = $this->db->table('schools')->where('id', $page_data['invoice_details']['school_id'])->get()->getRow();
    ...
    $stripe_row = $this->db->table('payment_settings')->where('school_id', $school_id)->where('key', 'stripe_settings')->get()->getRow();

    $paypal_row = $this->db->table('payment_settings')->where('school_id', $school_id)->where('key', 'paypal_settings')->get()->getRow();
  ...
  public function payment_success($payment_method = "", $invoice_id = "", $amount_paid = "", $reference = "") {
    ...
    if (!$is_community_payment && $current_user_type !== 'superadmin' && $invoice_school_id != $current_school_id) {
      log_message('error', "Unauthorized payment attempt for invoice #{$invoice_id}");
      session()->setFlashdata('error_message', get_phrase('access_denied'));
      return redirect()->to('/app/dashboard');
    }

    if ($invoice_details['status'] === 'paid') {
      log_message('error', "Double payment attempt for invoice #{$invoice_id}");
    ...
    $school = $this->db->table('schools')->where('id', $invoice_details['school_id'])->get()->getRowArray();
    ...
    if (empty($tax_residence) && $invoice_details['payment_type'] === 'subscription_admin') {
      $tax_residence = 'MA';
    }
    ...
    if ($invoice_details['payment_type'] === 'subscription_admin') {
      ...
      if ($payment_method == 'stripe' && !empty($stripe)) {
        $stripe_currency = $stripe[0]->stripe_currency ?? $invoice_currency;
    ...
    $expected_amount = $secure_amount;
    if ($has_vat_config && $invoice_details['payment_type'] === 'subscription_admin') {
      $expected_amount = round($secure_amount * (1 + $vat_rate / 100), 2);
    }
    ...
    $amounts_match = abs($amount_to_verify - $client_to_verify) <= ($amount_to_verify * $amount_tolerance);

    if (!empty($amount_paid) && !$amounts_match) {
      log_message('error', "Amount mismatch for invoice #{$invoice_id}: expected {$expected_amount}, got {$client_amount}, tolerance: {$amount_tolerance}");
      session()->setFlashdata('error_message', get_phrase('payment_amount_mismatch'));
      return redirect()->to('/app/payment/' . $invoice_id);
    }
    ...
    $token_id = request()->getPost('stripeToken');
    ...
    if ($payment_method == 'stripe') {
      ...
      if (!empty($secret_key) && !empty($token_id)) {
        $tax_residence_country = $school['country'] ?? '';
        $final_stripe_currency = $stripe_currency;
        $stripe_amount = $amount_paid;
        
        if ($tax_residence_country === 'UAE' || $tax_residence_country === 'AE') {
          $final_stripe_currency = 'AED';
          $stripe_amount = ($invoice_currency === 'AED') ? $secure_amount : $amount_paid;
        } elseif ($tax_residence_country === 'MA') {
          $final_stripe_currency = 'MAD';
          $stripe_amount = $secure_amount;
        }
      ...
      $payerID = request()->getPost('payerID');
      
      if (!empty($paymentID) && !empty($payerID)) {
        $payment_status = $this->validate_paypal_payment($paymentID, $payerID, $amount_paid, $gateway_school_id);
    ...
    if ($payment_status === true) {
      if ($invoice_details['payment_type'] === 'subscription_admin') {
        $invoice_current = $this->db->table('invoices')->where('id', $invoice_id)->get()->getRowArray();
        ...
        if ($due_amount <= $amount_for_comparison) {
          ...
          $payment_result = $invoiceModel->markInvoicePaid(
            $invoice_id, 
          ...
          if ($payment_result['success']) {
            $invoiceModel->clearSchoolCache($invoice_details['school_id']);
            log_message('info', "Subscription payment processed - Invoice #{$invoice_id} paid, school #{$invoice_details['school_id']} activated");
            session()->setFlashdata('flash_message', get_phrase('payment_successful_subscription_activated'));
          } else {
            log_message('error', "Subscription payment failed - Invoice #{$invoice_id}: " . $payment_result['message']);
            session()->setFlashdata('error_message', get_phrase('payment_processing_error'));
            return redirect()->to('/app/payment/' . $invoice_id);
          }
        ...
        $this->db->table('invoices')->where('id', $invoice_id)->update([
          'status' => 'paid',
        ...
        if ($is_community_payment) {
          ...
          $existing_admin = $this->db->table('admins')
            ->where('user_id', $user_id)
            ->where('school_id', $community_school_id)
            ->get()->getRow();
          ...
          session()->setFlashdata('flash_message', get_phrase('payment_successful_community_joined'));
          return redirect()->to('/app/dashboard');
        }
        
        session()->setFlashdata('flash_message', get_phrase('payment_successful'));
        return redirect()->to('/app/dashboard');
      }
    }

    session()->setFlashdata('error_message', get_phrase('payment_failed') ?: 'Le paiement a échoué');
    return redirect()->to('/app/payment/' . $invoice_id);
  }
  ...
  private function validate_paypal_payment($paymentID, $payerID, $expected_amount, $school_id) {
    try {
      ...
      if (empty($paypal_settings) || !isset($paypal_settings[0])) {
        log_message('error', 'PayPal: PayPal parameters not configured');
        return false;
      }
      ...
      if ($mode === 'sandbox') {
        $client_id = $paypal->paypal_test_client_id ?? '';
        $client_secret = $paypal->paypal_test_secret_key ?? '';
        $api_url = 'https://api-m.sandbox.paypal.com';
      } else {
        $client_id = $paypal->paypal_live_client_id ?? '';
        $client_secret = $paypal->paypal_live_secret_key ?? '';
        $api_url = 'https://api-m.paypal.com';
      }
      
      if (empty($client_id) || empty($client_secret)) {
        log_message('error', 'PayPal: Client ID or Secret missing');
        return false;
      }
      ...
      if ($http_code !== 200) {
        log_message('error', 'PayPal: Failed to get access token, HTTP code: ' . $http_code);
        return false;
      }
      ...
      if (empty($access_token)) {
        log_message('error', 'PayPal: Access token not received');
        return false;
      }

      $ch = curl_init();
      curl_setopt($ch, CURLOPT_URL, $api_url . '/v2/checkout/orders/' . $paymentID);
      curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
      ...
      if ($http_code !== 200) {
        log_message('error', 'PayPal: Failed to get order details, HTTP code: ' . $http_code);
        return false;
      }
      ...
      if (($order_data->status ?? '') !== 'COMPLETED') {
        log_message('error', 'PayPal: Order not completed, status: ' . ($order_data->status ?? 'unknown'));
        return false;
      }
      
      $purchase_unit = $order_data->purchase_units[0] ?? null;
      if (!$purchase_unit) {
        log_message('error', 'PayPal: No purchase unit found');
        return false;
      }
      
      $captures = $purchase_unit->payments->captures[0] ?? null;
      if (!$captures) {
        log_message('error', 'PayPal: No captures found');
        return false;
      }
      
      $paid_amount = (float) $captures->amount->value;
      $paid_currency = $captures->amount->currency_code;
      
      if ($paid_amount < $expected_amount) {
        log_message('error', "PayPal: Amount mismatch - expected {$expected_amount}, got {$paid_amount}");
        return false;
      }
      
      return true;
    } catch (Exception $e) {
      log_message('error', 'PayPal validation exception: ' . $e->getMessage());
      return false;
    }
  }
  ...
  public function expense_category($param1 = "", $param2 = "")
  {
    ...
    if ($param1 == 'update') {
      $modelResponse = $this->crud_model->update_expense_category($param2);
      $csrf = array(
        'csrfName' => csrf_token(),
        'csrfHash' => csrf_hash(),
      );

      return $this->response->setJSON(array('status' => $modelResponse['status'] ?? $modelResponse, 'csrf' => $csrf));
    }

    if ($param1 == 'delete') {
      $modelResponse = $this->crud_model->delete_expense_category($param2);
      $csrf = array(
        'csrfName' => csrf_token(),
        'csrfHash' => csrf_hash(),
      );
    ...
    if ($param1 == 'list') {
      return view('backend/admin/expense_category/list');
    }
  ...
  public function expense($param1 = "", $param2 = "")
  {
    ...
    // update expense
    if ($param1 == 'update') {
      $modelResponse = $this->crud_model->update_expense($param2);
      $csrf = array(
        'csrfName' => csrf_token(),
        'csrfHash' => csrf_hash(),
      );
    ...
    // deleting expense
    if ($param1 == 'delete') {
      $modelResponse = $this->crud_model->delete_expense($param2);
      $csrf = array(
        'csrfName' => csrf_token(),
        'csrfHash' => csrf_hash(),
      );
    ...
    if ($param1 == 'list') {
      ...
      return view('backend/admin/expense/list', $page_data);
    }
  ...
  public function session_manager($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'reopen_list') {
      return view('backend/admin/session/table_body');
    }

    if ($param1 == 'list') {
      return view('backend/admin/session/list');
    }
  ...
  public function book($param1 = "", $param2 = "")
  {
    ...
    if ($param1 == 'list') {
      return view('backend/admin/book/list');
    }
  ...
  public function book_issue($param1 = "", $param2 = "")
  {
    ...
    if ($param1 == 'list') {
      ...
      $page_data['date_from'] = strtotime($date[0] . ' 00:00:00');
      $page_data['date_to'] = strtotime($date[1] . ' 23:59:59');
      return view('backend/admin/book_issue/list', $page_data);
    }
  ...
  public function addon_manager($param1 = "", $param2 = "")
  {
    ...
    // showing the list of book
    if ($param1 == 'list') {
      return view('backend/admin/addon/list');
    }
    // showing the index file
    if (empty($param1)) {
  ...
  public function noticeboard($param1 = "", $param2 = "", $param3 = "")
  {
    ...
    if ($param1 == 'list') {
      return view('backend/admin/noticeboard/list');
    }
  ...
  public function website_update($param1 = "")
{
  ...
  public function events($param1 = "", $param2 = "")
  {
    ...
    // showing the list of book
    if ($param1 == 'list') {
      return view('backend/admin/website_settings/events');
    }
  ...
  public function frontend_gallery($param1 = "", $param2 = "", $param3 = "")
  {
    ...
    if ($param1 == 'gallery_photo_list') {
      $page_data['gallery_id'] = $param2;
      return view('backend/admin/website_settings/gallery_image', $page_data);
    }
  ...
  public function school_settings($param1 = "", $param2 = "")
  {
    ...
    $is_admin = session()->get('admin_login') == 1;
    $is_superadmin = session()->get('superadmin_login') == 1;

    if (!$is_admin && !$is_superadmin) {
      return $this->response->setStatusCode(403)->setJSON([
        'status' => false,
        'notification' => 'Access denied. Admin privileges required.'
      ]);
    }
  ...
  public function language($param1 = "", $param2 = "")
  {
    ...
    if ($param1 == 'list') {
      return view('backend/admin/language/list');
    }
 ...
 if ($param1 == 'active') {
    ...
    if ($param1 == 'dropdown') {
      return view('backend/admin/language/dropdown');
    }
    // showing the index file
    if (empty($param1)) {
  ...
  public function exam_results($exam_id = "", $student_id = "") {
    try {
        ...
        if (!session()->get('admin_login') || session()->get('user_type') != 'admin') {
            return redirect()->to(site_url('login'));
        }
...
public function filter_exams()
{
    ...
    foreach ($exams as $exam) {
        $exam_data[] = [
            ...
            'formatted_date' => $exam['starting_date'] ? date('D, d-M-Y H:i', strtotime($exam['starting_date'])) : 'No Date',
...
public function calendar($param1 = '', $param2 = '', $param3 = '', $param4 = '') {
    ...
    log_message('debug', 'calendar() called - admin_login: ' . var_export($admin_login, true) . ', user_type: ' . var_export($user_type, true) . ', session_id: ' . var_export($session_id, true) . ', all_session: ' . var_export($all_session, true));
...
public function create_event() {
    ...
    // GET request - redirect to calendar page
    ...
    // Validation des participants (optionnel - autoriser les événements sans participants)
    ...
    // Les participants sont maintenant optionnels - on continue même si c'est vide
    ...
    try {
        ...
        $audit_log_model = new \App\Models\Audit_log_model();
        $audit_log_model->log_event_create(
            session()->get('user_id'),
            $event_id,
            array_merge($data, ['participants_count' => count($participants)])
        );
        echo json_encode([
            'status' => 'success',
...
public function update_event() {
    ...
    $admin_login = session()->get('admin_login');
    log_message('debug', 'get_events() called - admin_login: ' . var_export($admin_login, true));
    ...
    if ($class_id) {
        $sql .= " AND EXISTS (SELECT 1 FROM participants WHERE event_calendars.id = participants.event_id AND participants.guest = ? AND participants.type = 'class')";
        $params[] = $class_id;
    }
...
public function start_meeting(){
    ...
    $this->superadmin_model->update_table('appointments', ['event_id' => $event_id, 'Etat' => 1], ['Etat' => 0]);
    db()->query("UPDATE appointments SET `Etat` = 0 WHERE event_id = ? AND DATE(start_date) != ? AND `Etat` = 1", [$event_id, $occurrence_date]);

    // VÃ©rifier si un appointment actif existe pour cette occurrence
    $appointment = $this->superadmin_model->get_row('appointments', ['event_id' => $event_id, 'Etat' => 1]);
    $appointment_id = $appointment ? $appointment['id'] : null;
    
    if ($appointment) {
        $appointments = db()->query("SELECT * FROM appointments WHERE event_id = ? AND DATE(start_date) = ? AND `Etat` = 1", [$event_id, $occurrence_date])->getRowArray();
        if ($appointments) {
...
public function get_school_data() {
  ...
  public function check_teacher_email()
  {
    ...
    if ($email === '') {
      return $this->response->setJSON(['status' => 'new', 'csrf' => $csrf]);
    }

    // Handle legacy data where the same email may exist on multiple users.
    ...
    if (!empty($userIds) && $school_id > 0) {
      $alreadyTeacher = $this->db->table('user_schools')
        ->whereIn('user_id', $userIds)
        ->where('school_id', $school_id)
        ->where('role', 'teacher')
        ->countAllResults();

      if ((int) $alreadyTeacher > 0) {
        return $this->response->setJSON([
    ...
    // Pick the best candidate user when duplicate emails exist:
    // 1) already linked to current school, 2) highest number of communities, 3) latest id.
    $bestUser = $users[0];
    $bestScore = -1;
    foreach ($users as $candidate) {
      ...
      $linkedToCurrentSchool = 0;
      if ($school_id > 0) {
        $linkedToCurrentSchool = (int) $this->db->table('user_schools')
          ->where('user_id', $candidateId)
          ->where('school_id', $school_id)
          ->countAllResults();
      }

      $communityCount = (int) $this->db->table('user_schools')
        ->where('user_id', $candidateId)
        ->countAllResults();

      $score = ($linkedToCurrentSchool > 0 ? 100000 : 0) + ($communityCount * 100) + $candidateId;
      if ($score > $bestScore) {
        $bestScore = $score;
        $bestUser = $candidate;
      }
    ...
    return $this->response->setJSON([
      ...
      'user' => [
        'id' => (int) ($user['id'] ?? 0),
        'name' => html_entity_decode((string) ($user['name'] ?? '')),
  ...
  public function billing_entities($param1 = '', $param2 = '')
  {
    ...
    if ($param1 == 'save_credentials' && !empty($param2)) {
      $provider = $this->request->getUri()->getSegment(5); // /superadmin/billing_entities/save_credentials/{id}/{provider}
  ...
  private function _billing_entity_list()
  {
    ...
    return view('backend/admin/billing_entities/list', $data);
  }
  ...
  private function detect_user_country() {
    ...
    if ($user_ip === '127.0.0.1' || $user_ip === '::1') {
      $cf_country = $this->request->getServer('HTTP_CF_IPCOUNTRY');
      if (!empty($cf_country)) {
    ...
    try {
      ...
      log_message('error', 'GeoIP error: ' . $e->getMessage());
    }
    return 'UNKNOWN';
  }
  ...
  private function get_conversion_rate($from, $to) {
    ...
    $cached_rate = cache()->get("fx_rate_{$from}_{$to}");
    if ($cached_rate !== null) {
      return $cached_rate;
    }
    ...
    try {
      ...
      if ($response) {
        ...
        cache()->save("fx_rate_{$from}_{$to}", $rate, 3600);
        return $rate;
      }
    } catch (Exception $e) {
      log_message('error', 'FX rate error: ' . $e->getMessage());
    }
    return 1.0;
  }
  ...
  private function get_stripe_currency($school_id = 1) {
    ...
    if ($stripe_row) {
      $stripe = is_string($stripe_row->value) ? json_decode($stripe_row->value, true) : (array) $stripe_row->value;
      return $stripe[0]['stripe_currency'] ?? 'USD';
    }
    return 'USD';
  }
}
