...
class FxRatesService {
    ...
    public function __construct()
    {
        ...
        $fxratesConfig = config('FxRates');
        
        $this->config = [
            'fxrates_api_key' => $fxratesConfig->fxrates_api_key ?: '',
            'fxrates_api_url' => $fxratesConfig->fxrates_api_url ?: 'https://v6.exchangerate-api.com/v6/',
            'fxrates_base_currency' => $fxratesConfig->fxrates_base_currency ?: 'USD',
            'fxrates_currencies' => $fxratesConfig->fxrates_currencies ?: ['USD', 'EUR', 'MAD', 'AED'],
            'fxrates_cache_duration' => $fxratesConfig->fxrates_cache_duration ?: 600,
            'fxrates_cache_prefix' => $fxratesConfig->fxrates_cache_prefix ?: 'fxrates_',
            'fxrates_api_timeout' => $fxratesConfig->fxrates_api_timeout ?: 30,
            'fxrates_api_connect_timeout' => $fxratesConfig->fxrates_api_connect_timeout ?: 10,
            'fxrates_store_raw_json' => $fxratesConfig->fxrates_store_raw_json !== null ? $fxratesConfig->fxrates_store_raw_json : true,
            'fxrates_timezone' => $fxratesConfig->fxrates_timezone ?: 'UTC',
            'fxrates_api_token' => $fxratesConfig->fxrates_api_token ?: '',
            'fxrates_cron_token' => $fxratesConfig->fxrates_cron_token ?: ''
        ];
    ...
    public function upsertDailyRates($date, $rates, $raw_json = null)
    {
        ...
        $result = model(App\Models\FxRates_model)->upsert($data);
    ...
    public function getRatesByDate($date)
    {
        ...
        // Get from database
        $record = model(App\Models\FxRates_model)->get_by_date($date);
        
        if ($record) {
            $formatted = model(App\Models\FxRates_model)->format_for_response($record);
    ...
    public function getLatestRates()
    {
        ...
        // Get from database
        $record = model(App\Models\FxRates_model)->get_latest();
    ...
    public function getRange($start_date, $end_date)
    {
        ...
        $records = model(App\Models\FxRates_model)->get_range($start_date, $end_date);
        
        $result = [];
        foreach ($records as $record) {
            $result[] = model(App\Models\FxRates_model)->format_for_response($record);
        }
    ...
    public function fetchAndStoreTodayRates()
    {
        ...
        if ($rates === false) {
            ...
            model(App\Models\FxRates_model)->update_status($today, 'stale');
    ...
    public function getHealthStatus()
    {
        ...
        $has_today_rates = model(App\Models\FxRates_model)->has_today_rates();
        $latest = model(App\Models\FxRates_model)->get_latest();
        $total_records = model(App\Models\FxRates_model)->count_all();
