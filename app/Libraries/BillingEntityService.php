...
class BillingEntityService {
    ...
    public function __construct()
    {
        try {
            ...
            $this->db = null;
            log_message('error', 'BillingEntityService: Failed to connect to database - ' . $e->getMessage());
        }
    }
    ...
    private function getModel()
    {
        ...
        $this->model = model('BillingEntity_model');
        if (!is_object($this->model)) {
            $this->model = model('App\Models\BillingEntity_model');
        }
    ...
    public function is_available()
    {
        ...
        try {
            ...
            if (method_exists($model, 'tables_exist')) {
                return (bool) $model->tables_exist();
            }

            return (bool) $this->db->tableExists('billing_entities');
        } catch (\Throwable $e) {
            return false;
        }
    }
    ...
    public function get_entity_for_tax_residence($tax_residence)
    {
        ...
        if ($this->is_available()) {
            try {
                $entity = $this->getModel()->get_by_tax_residence($tax_residence);
                if ($entity) {
                    return $this->getModel()->format_for_ui($entity);
                }
            } catch (\Throwable $e) {
    ...
    public function get_all_entities()
    {
        if ($this->is_available()) {
            try {
                $entities = $this->getModel()->get_all();
                $formatted = [];
                foreach ($entities as $entity) {
                    $formatted[] = $this->getModel()->format_for_ui($entity);
                }
                return $formatted;
            } catch (\Throwable $e) {
    ...
    public function get_all_active()
    {
        if ($this->is_available()) {
            try {
                $entities = $this->db->table('billing_entities')
                    ->where('is_active', 1)
    ...
    public function create_entity($data)
    {
        ...
        try {
            $entity_id = $this->getModel()->create($data);
    ...
    public function get_for_admin()
    {
        ...
        try {
            return $this->getModel()->get_for_admin();
        } catch (\Throwable $e) {
