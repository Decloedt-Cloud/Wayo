...
class BillingEntityCredentials_model extends Model {
    ...
    {
        return \db()->table(self::TABLE)->where('billing_entity_id', $entity_id)->get()->getResultArray();
    }
    ...
    public function save_credentials($entity_id, $provider, $data)
    {
        ...
        if ($existing) {
            return \db()->table(self::TABLE)->where('id', $existing['id'])->update($save_data);
        } else {
            return \db()->table(self::TABLE)->insert($save_data);
        }
    }
    ...
    public function get_credentials_for_tax_residence($tax_residence, $provider)
    {
        ...
        if (!$entity) {
            ...
            $entity = $billingEntityModel->get_default();
        }
    ...
    public function get_payment_credentials_for_school($school_id, $provider)
    {
        ...
        $school = \db()->table('schools')->get()->getResult()->getRowArray();
    ...
    private function _get_legacy_settings($school_id, $provider)
    {
        ...
        $row = \db()->table('payment_settings')->get()->getResult()->getRow();
