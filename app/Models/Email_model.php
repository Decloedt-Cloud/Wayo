...
class Email_model extends Model {
	...
	{
		$query = \db()->table('users')->where(['id' => $user_id])->get()->getResultArray();
		if(sizeof($query) > 0)
		{
			$query = $query[0];
	...
	function password_reset_email_link($link = '' , $user_id = "")
	{
		...
		if(sizeof($query) > 0)
		{
			...
			$contact_url = site_url('home/contact#map');
			$current_year = date('Y');
								...
								<table>
									<tr>
										<td style="text-align: left; width: 33%;">
											<a href="$contact_url" style="color: #fff; text-decoration: none;">$address</a>
										</td>
										<td style="text-align: center; width: 34%;">
											© $current_year Wayo Academy. All rights reserved.
										</td>
										<td style="text-align: right; width: 33%;">
											Tel: <a href="$phone" style="color: #fff; text-decoration: none;">$phone</a>
										</td>
									</tr>
								</table>
							</td>
    ...
    if (!empty($query)) {
		...
		<head>
			<meta charset="UTF-8">
			<meta name="viewport" content="width=device-width, initial-scale=1.0">
			<title>Finalize Your Account Creation</title>
			<style>
				body { 
					margin: 0; 
					padding: 0; 
		...
		<body>
			<div class="container">
				...
				<table class="content-table">
					<tr>
						<td class="header">
							<span>Finalize Your Account Creation</span>
						</td>
					</tr>
					<tr>
						<td class="body">
							...
							<p>Please click the button below to set your password and access your account to explore our services:</p>
							<div class="button-container">
        ...
        $email_sub = "Finalize Your Account Creation";
        $email_to = $query['email'];
	...
	function personal_message_email($email_from, $email_to, $email_message) {
		$email_sub = "Message from Community Website";
	...
	function request_book_email($student_id){
		...
		$librarians = \db()->table('librarian')->limit(1)->get()->getResultArray();
	...
	function Add_online_admission($email = "", $user_id = "",$name = ""){
		...
		// $image_url = "http://51.92.7.185/uploads/schools/".$school_data['id'].".jpg";
		$systemEmail = get_settings('system_email');
		$login = site_url('home/communities');
		$contact_url = site_url('home/contact#map');
		$current_year = date('Y');
		$phone = get_settings('phone');
		$address = get_settings('address');
						...
						<td class="body">
							<div class="info-container">
								<div class="info-content">
									...
									<p>If you have any questions or need assistance, please feel free to contact our support team at: <a href="mailto:{$systemEmail}">{$systemEmail}</a>.</p>
									<div class="button-container">
										<a href="$login" class="button">Discover our Communities</a>
									</div>
								</div>
							</div>
						</td>
	...
	function School_online_admission($email = "", $school_name = "", $name = "") {
		...
		<head>
			<meta charset="UTF-8">
			<meta name="viewport" content="width=device-width, initial-scale=1.0">
			<title>Registration Community</title>
			<style>
				body { 
					margin: 0; 
					padding: 0; 
		...
		<body>
			...
			<table class="content-table">
				<tr>
					<td class="header">
						<span>Confirmation of Your Account and Community Creation</span>
					</td>
				</tr>
				<tr>
					<td class="body">
						<div class="info-container">
							<div class="info-content">
								...
								<p>We are pleased to confirm that your account has been successfully created. Congratulations on setting up your community!</p>
								<p><strong>Community:</strong> {$school_name}</p>
								<p><strong>Email:</strong> {$email}</p>
								<p>Please note that the creation of your community is currently under verification process. Once the verification is complete, your community will be visible among other communities on the platform.</p>
		...
		$email_sub = 'Registration Community';
		$email_to = $email;
	...
	function School_online_admission_superadmin($email = "", $school_name = "", $name = "") {
		...
		<head>
			<meta charset="UTF-8">
			<meta name="viewport" content="width=device-width, initial-scale=1.0">
			<title>New Community Creation Notification</title>
			<style>
				body { 
					margin: 0; 
					padding: 0; 
		...
		<body>
			<div class="container">
				...
				<table class="content-table">
					<tr>
						<td class="header">
							<span>New Community Creation Notification</span>
						</td>
					</tr>
					<tr>
						<td class="body">
							<p><strong>Dear Superadmin,</strong></p>
							<p>We are pleased to inform you that a new Community has been successfully registered on the Wayo Academy platform.</p>
							<p>Below are the details of the new Community and its administrator:</p>
							<p><strong>Community Name:</strong> $school_name</p>
		...
		$email_sub = 'Application for Community Admission';
		$email_to = $systemEmail;
		$school_name = "Wayo Academy";
	
		$this->send_mail_using_smtp($email_msg, $email_sub, $email_to, null, $school_name);
	}
	...
	function approved_online_admission($student_id = "", $user_id = "")
	{
				...
				<table class="content-table">
					...
					<tr>
						<td class="body">
							...
							<p>We are pleased to inform you that your admission request has been successfully approved.</p>
							<p>Below are the details of your admission:</p>
							<p><strong>Member Code:</strong> $student_code</p>
							<p><strong>Member Name:</strong> $student_name</p>
		...
		HTML;
				$email_sub = 'Admission Approved';
				$email_to = $student_email;
				

				$this->send_mail_using_smtp($email_msg, $email_sub, $email_to,Null,$school_data['name']);
	}


	function join_student_email($user_name, $email_student = "", $code_student = "", $name_school = "", $school_id = "")
	{
		// $image_url = "http://51.92.7.185/uploads/schools/".$school_id.".jpg";
		$systemEmail = get_settings('system_email');
		$login = site_url('login');
		$contact_url = site_url('home/contact#map');
		$current_year = date('Y');
				...
				<table class="content-table">
					...
					<tr>
						<td class="body">
							...
							<p>Thank you for submitting your request to join <strong>$name_school</strong>.</p>
							...
							<p>You will be notified once your request has been reviewed and approved.</p>
							<p>In the meantime, you may log in to your account to check the status of your application or access additional information</p>
							<p>If you have any questions or need assistance, please feel free to contact our support team at: <a href="mailto:{$systemEmail}">{$systemEmail}</a></p>
							<div class="button-container">
								<a href="$login" class="button">Login to Your Account</a>
							</div>
		...
		HTML;
				$email_sub = 'Join Request Confirmation';
				$email_to = $email_student;
				

				$this->send_mail_using_smtp($email_msg, $email_sub, $email_to,Null,$name_school);
	}
	function join_student_email_for_admin($user_name, $email_student = "", $code_student = "", $name_school = "", $school_id = "", $name = "")
		{
				// $image_url = "http://51.92.7.185/uploads/schools/".$school_id.".jpg";
				$systemEmail = get_settings('system_email');
				$login = site_url('login');
		...
		<body>
			<div class="container">
				...
				<table class="content-table">
					<tr>
						<td class="header">
							<span>New registration in your community</span>
						</td>
					</tr>
					<tr>
						<td class="body">
							<p><strong>Dear $name,</strong></p>
							<p>We are pleased to inform you that a new registration has been submitted for $name_school</p>
							<p>Below are the details of the registrant:</p>
		...
		HTML;
				...
				$email_sub = 'New Registration Notification';
				$email_to = $email_student;
	...
	function approved_online_admission_parent_access($user_id = "", $password = ""){
		$parent_details = \db()->table('users')->where(['id' => $user_id])->get()->getResultArray();
    ...
    $email->setTo($to);
    $email->setFrom($from, $school_name ?? get_settings('system_name'));
    $email->setSubject($sub);
    $email->setMessage($htmlContent);

    // Send email and handle errors
    if (!$email->send()) {
        log_message('error', 'CI Email Debugger: ' . $email->printDebugger(['headers', 'subject', 'body']));
        return false;
    }
	...
	function send_email_verification($email, $verification_token, $user_name) {
		$system_name = get_settings('system_name');
		$verification_url = base_url('verify-email/' . $verification_token);
		$current_year = date('Y');
...
<head>
    ...
    <style>
        ...
        .container { max-width: 600px; margin: 0 auto; background-color: #ffffff; padding: 20px; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
        .header { background-color: #4a90e2; color: #ffffff; padding: 20px; text-align: center; border-radius: 5px 5px 0 0; }
        .header h1 { margin: 0; font-size: 24px; }
        .content { padding: 30px 20px; }
        .content p { line-height: 1.6; color: #333333; }
        .button { display: inline-block; background-color: #4a90e2; color: #ffffff; padding: 12px 30px; text-decoration: none; border-radius: 5px; margin-top: 20px; }
...
<body>
    <div class="container">
        ...
        <div class="content">
            <p>Dear {$user_name},</p>
            <p>Thank you for registering with {$system_name}. To complete your registration and activate your account, please verify your email address by clicking the button below:</p>
            ...
            <p>If you did not create an account with {$system_name}, please ignore this email.</p>
        </div>
        <div class="footer">
...
HTML;
		...
		$email_sub = "Verify Your Email Address";
		
		// Sending the email using SMTP or PHPMailer
		if (get_smtp('mail_sender') == 'php_mailer') {
			return $this->send_mail_using_php_mailer($email_msg, $email_sub, $email);
		} else {
			return $this->send_mail_using_smtp($email_msg, $email_sub, $email);
		}
	}
