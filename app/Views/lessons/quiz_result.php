...
$lms_model = (isset($this->lms_model) && is_object($this->lms_model))
    ? $this->lms_model
    : model('App\\Models\\addons\\Lms_model');

$to_row_array = static function ($result): array {
    if (is_array($result)) {
        return $result;
    }
    if (is_object($result)) {
...
<div class="result-wrapper">
    ...
    <div class="answers-section">
        ...
        <?php foreach ($submitted_quiz_info as $index => $each):
            $question_details = $to_row_array($lms_model->get_quiz_question_by_id($each['question_id']));
        ...
        <div class="answer-card <?php echo $is_correct ? 'correct' : 'incorrect'; ?>" style="animation-delay: <?php echo ($index * 0.05); ?>s;">
            ...
            <div class="answer-header">
                ...
                <span class="answer-question">
                    <?php echo get_phrase('question'); ?> <?php echo $index + 1; ?>: <?php echo $question_details['title'] ?? get_phrase('question'); ?>
                </span>
            </div>
