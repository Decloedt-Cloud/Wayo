...
$lesson_result = (is_object($lms_model) && method_exists($lms_model, 'get_lessons'))
    ? $lms_model->get_lessons('lesson', $lesson_id)
    : [];
if (is_object($lesson_result) && method_exists($lesson_result, 'row_array')) {
    $lesson_details = $lesson_result;
} elseif (is_object($lesson_result) && method_exists($lesson_result, 'getRowArray')) {
    $lesson_details = $lesson_result->getRowArray();
} elseif (is_array($lesson_result)) {
    $lesson_details = $lesson_result[0] ?? $lesson_result;
} else {
    $lesson_details = [];
}
if (is_object($lesson_details)) {
    $lesson_details = (array) $lesson_details;
}
if (!is_array($lesson_details)) {
    $lesson_details = [];
}
...
$course_row = (is_object($lms_model) && method_exists($lms_model, 'get_course_by_id'))
    ...
    : [];
$course_thumbnail = is_array($course_row) ? ($course_row['thumbnail'] ?? 'placeholder.png') : 'placeholder.png';
$lesson_thumbnail_url = 'uploads/course_thumbnail/' . $course_thumbnail;
...
$opened_section_id = $lesson_details['section_id'] ?? null;
?>
