...
$quiz_questions_result = (is_object($lms_model) && method_exists($lms_model, 'get_quiz_questions'))
    ? $lms_model->get_quiz_questions($lesson_details['id'])
    : [];
if (is_array($quiz_questions_result)) {
    $quiz_questions = $quiz_questions_result;
} elseif (is_object($quiz_questions_result) && method_exists($quiz_questions_result, 'result_array')) {
    $quiz_questions = $quiz_questions_result;
} elseif (is_object($quiz_questions_result) && method_exists($quiz_questions_result, 'getResultArray')) {
    $quiz_questions = $quiz_questions_result->getResultArray();
} else {
    $quiz_questions = [];
}
...
$has_previous_attempt = $db->table('quiz_responses')
...
<div class="quiz-wrapper">
    ...
    <form id="quiz_form" action="" method="post">
        ...
        <?php if ($question_count > 0): ?>
            <?php foreach ($quiz_questions as $key => $quiz_question):
