...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function getActiveVideoElements() {
      const allVideos = document.querySelectorAll('video.plyr-video');
      if (isDesktop()) {
        return Array.from(allVideos).filter(v => v.id.includes('desktop'));
      } else {
        return Array.from(allVideos).filter(v => v.id.includes('mobile'));
      }
    }
    
    // 🚀 Initialiser et lancer la vidéo immédiatement au chargement
    const activeVideos = getActiveVideoElements();
