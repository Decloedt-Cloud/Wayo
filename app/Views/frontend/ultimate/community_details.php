              ...
              <?php if (!empty($classes)): ?>
                <?php 
                  ...
                  if ($is_logged_in) {
                      ...
                      $admin_check = db()->table('user_schools')->where('user_id', $user_id)->where('school_id', $school_id)->where('role', 'admin')->get()->getResultArray();
                      // Check for teacher role
                      $teacher_check = db()->table('user_schools')->where('user_id', $user_id)->where('school_id', $school_id)->where('role', 'teacher')->get()->getResultArray();
                      
                      if (count($admin_check) > 0 || count($teacher_check) > 0) {
                  ...
                  if ($is_authority) {
                      $is_teacher_role = (isset($teacher_check) && count($teacher_check) > 0) || ($is_teacher && $session_school_id == $school_id);
                ...
                <?php foreach ($classes as $key => $class): ?>
                  <?php
                    ...
                    $classPhoto = (string) ($class['photo'] ?? '');
                    $classDesc = (string) ($class['description'] ?? '');
                    $classMentor = (string) ($class['mentor'] ?? '');
                    $classDuration = (string) ($class['duration'] ?? '');
                    $classLevel = (string) ($class['level'] ?? '');
                    $classStart = (string) ($class['date_debut'] ?? '');
                    $classEnd = (string) ($class['date_fin'] ?? '');
                    $classCycle = (string) ($class['cycle'] ?? '');
                    $classMembersMax = (string) ($class['nombre_max_membre'] ?? '');
                    $classPrice = (float) ($class['price'] ?? 0);
                    $classCurrency = (string) (($class['currency'] ?? '') ?: $currencies);
                  ?>
                  <div class="col">
                    <div class="class-card"
                      data-title="<?php echo htmlspecialchars($className); ?>"
                      data-photo="<?php echo htmlspecialchars($classPhoto); ?>"
                      data-desc="<?php echo htmlspecialchars($classDesc); ?>"
                      data-mentor="<?php echo htmlspecialchars($classMentor); ?>"
                      data-duration="<?php echo htmlspecialchars($classDuration); ?>"
                      data-level="<?php echo htmlspecialchars($classLevel); ?>"
                      data-start="<?php echo htmlspecialchars($classStart); ?>"
                      data-end="<?php echo htmlspecialchars($classEnd); ?>"
                      data-price="<?php echo $classPrice; ?>"
                      data-currency="<?php echo $currencies; ?>"
                      data-cycle="<?php echo htmlspecialchars($classCycle); ?>"
                      data-free="<?php echo htmlspecialchars($classMembersMax); ?>">

                      <div class="card-body">
                        <div class="class-header">
                          <h4 class="class-title"><?php echo htmlspecialchars($className); ?></h4>
                          <span class="price-badge">
                            <?php
                            if ($classPrice > 0) {
                              $class_price_ttc = isset($classes_with_vat[$key]['price_ttc']) ? $classes_with_vat[$key]['price_ttc'] : $classPrice;
                              echo number_format($class_price_ttc, 2) . ' ' . $classCurrency;
                              if ($vat_rate > 0) {
                     ...
                     <button
                        ...
                        data-class-enrolled="<?php
                            echo count(db()->table('enrols')->where([
                        ...
                        data-user-role="<?php echo session()->get('admin_login') == 1 ? 'admin' : (session()->get('teacher_login') == 1 ? 'teacher' : ''); ?>"
                      ...
                      >
                        <?php echo get_phrase("See more"); ?>
                          ...
                          <?php
                            ...
                            $enrols_datas = 0;
                            if ($student_id > 0) {
                                $enrols_datas = db()->table('enrols')->where('student_id', $student_id)->where('school_id', $school_id)->where('class_id', $class['id'])->get()->getNumRows();
                            }

                            $enrols_max = db()->table('enrols')->where('school_id', $school_id)->where('class_id', $class['id'])->get()->getNumRows();
                            ...
                            if ($is_authority): 
                                $role_to_switch = ($is_teacher || (isset($teacher_check) && count($teacher_check) > 0)) ? 'teacher' : 'admin';
                            ?>
                            ...
                            // CASE 3 : pas encore inscrit
                              ...
                              if ($status == -1): ?>
                                ...
                                <form action="<?php echo base_url('student/join_school/assigned/' . $school_id); ?>" method="post">
                                  ...
                                  <button type="submit" class="btn btn-wayo fw-bold <?php if(!session()->get('user_id')) echo 'join-community-login-popup'; ?>">
                                    <?php 
...
const currentSchoolId = "<?php echo session()->get('active_school_id'); ?>";
const currentRole = "<?php echo session()->get('role'); ?>";
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.querySelectorAll('.openClassModal').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      const userRole = this.dataset.userRole || "<?php echo session()->get('admin_login') == 1 ? 'admin' : (session()->get('teacher_login') == 1 ? 'teacher' : ''); ?>";
