...
?>
    <div class="exp-empty-state">
        ...
        <div class="exp-empty-text"><?php echo get_phrase('no_classes_found'); ?></div>
        <p class="text-muted mt-2"><?php echo get_phrase('add_classes_to_this_school_to_manage_permissions'); ?></p>
    </div>
<?php
    } else {
        // Use a direct CI4 query builder here to avoid CI3 DB shim alias issues in views.
        $db = \Config\Database::connect();
        $teachers = $db->table('teachers t')
            ->select('t.id as teacher_id, t.user_id, t.designation, u.name, u.email, tp.marks, tp.attendance')
            ->join('users u', 't.user_id = u.id')
            ->join('teacher_permissions tp', 't.id = tp.teacher_id AND tp.class_id = ' . $db->escape($class_id), 'left')
...
<div class="exp-list-header">
    ...
    <div class="justify-content-center text-center">
        <span data-bs-toggle="tooltip" title="<?php echo get_phrase('permission_history'); ?>">
            <i class="mdi mdi-history me-1"></i><?php echo get_phrase('history'); ?>
        </span>
    </div>
</div>
...
<div class="exp-list-body">
    ...
    <div class="exp-list-item <?php echo $is_all ? 'bg-soft-success' : ''; ?>">
        <div class="exp-teacher-info ps-2">
            ...
            <div>
                <div class="exp-teacher-name"><?php echo esc($teacher['name']); ?></div>
                <div class="small text-muted d-flex align-items-center gap-1">
                    <i class="mdi mdi-email-outline" style="font-size: 12px;"></i> <?php echo esc($teacher['email']); ?>
                </div>
                <?php if(!empty($teacher['designation'])): ?>
                    <div class="badge badge-soft-primary mt-1" style="font-size: 10px;"><?php echo esc($teacher['designation']); ?></div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="text-center d-flex justify-content-center align-items-center">
            <button type="button" 
                    ...
                    onclick="viewPermissionHistory(<?php echo $teacher['teacher_id']; ?>)"
                    title="<?php echo get_phrase('view_permission_history'); ?>">
...
<script>
    ...
    if (typeof bootstrap !== 'undefined') {
        ...
        <?php endif; ?>
    <?php } // End of else block for class_id check ?>