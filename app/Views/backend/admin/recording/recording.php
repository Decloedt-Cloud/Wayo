...
<style>
/* ============================================================================
   RECORDING MANAGER - MODERN DESIGN (MATCHING TEACHER/STUDENT STYLE)
   ============================================================================ */
...
$(document).ready(function() {
    ...
    function showNotification(type, message, duration = 3000) {
        <?php if (!(isset($enable_toasts) ? (bool) $enable_toasts : true)): ?>
            return;
        <?php endif; ?>
        Swal.fire({
            toast: true,
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'success') {
                    $('input[name="<?php echo csrf_token(); ?>"]').val(response.csrf_token);
    ...
    $('#clearFilters').on('click', function(event) {
        ...
        var formData = {
            meeting_name: '',
            date_range: '',
            "<?php echo csrf_token(); ?>": $('input[name="<?php echo csrf_token(); ?>"]').val()
        };
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            ...
                            // Re-init list logic (update counts etc)
                            // We need to trigger the global re-init manually if possible or just let it be
                            // The easiest is to trigger a custom event or just recall collectItems if exposed.
                            // Since we put logic in IIFE, we can't access it easily unless we expose it.
                            // But we exposed window.updateRecordingList.
                            // Let's just remove the element. The pagination counts will be off until refresh, 
                            // unless we expose a "refresh" method.
                            // Actually, let's just reload the list via filter to be safe/clean
