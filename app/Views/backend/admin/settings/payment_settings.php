...
<?php
$settings_model = model('Settings_model');
...
$school_data = $settings_model->get_current_school_data();
$settings_school = $settings_model->get_current_settings_school_data();
...
if (!$settings_school) {
    $settings_school = ['type' => ''];
}
if (!$result) {
    $result = ['vat' => 0, 'system_currency' => '', 'vat_enabled' => 0, 'vat_rate' => 0];
}
                ...
                <form method="POST" class="systemvatAjaxForm" action="<?php echo route('payment_settings/vat'); ?>" id="vat_settings">
                    ...
                    <div class="form-group-modern">
                        ...
                        <select class="form-control-modern" name="vat_applicable" id="vat_applicable">
                            ...
                            <option value="1" <?php if (($result['vat_enabled'] ?? 0) == 1) echo 'selected'; ?>><?php echo get_phrase('Yes'); ?></option>
                            <option value="0" <?php if (($result['vat_enabled'] ?? 0) == 0) echo 'selected'; ?>><?php echo get_phrase('No'); ?></option>
                        </select>
                    </div>
    ...
    $(".paypalAjaxForm, .systemAjaxForm, .stripeAjaxForm, .systemvatAjaxForm, .systempriceAjaxForm").submit(function(e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var csrfName = csrf.name || csrf.csrfName;
                var csrfHash = csrf.hash || csrf.csrfHash;
                if (csrfName && csrfHash) {
                    $('input[name="' + csrfName + '"]').val(csrfHash);
                }

                if (response.status) {
                    success_notify(response.notification || '<?= js_phrase(get_phrase('updated_successfully')); ?>');
                    setTimeout(function() {
                        location.reload();
                    }, 2000);
                } else {
                    error_notify(response.notification || '<?= js_phrase(get_phrase('action_not_allowed')); ?>');
