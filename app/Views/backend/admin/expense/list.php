...
if (isset($expense_category_id) && $expense_category_id > 0) {
    if ($expense_category_id != 'all') {
        $expenses = $this->crud_model->get_expense($date_from, $date_to, $expense_category_id);
    } else {
...
$school_currency = $this->db->table('settings_school')->where('school_id', school_id())->get()->getRow('system_currency') ?? 'EUR';
?>
        ...
        <div class="exp-list-item" 
            ...
            <div class="exp-col-amount">
                <div class="exp-amount-display">
                  <?php $school_currency = db()->table('settings_school')->where('school_id', school_id())->get()->getRowArray()['system_currency'] ?? '' ?? 'EUR'; ?>
