...
if (isset($student_list_rows) && is_array($student_list_rows)) {
    // Preferred CI4 path: use preloaded data when provided by controller.
    $students_data = $student_list_rows;
    ...
    $classes = isset($student_list_classes) && is_array($student_list_classes)
        ? $student_list_classes
        : db()->table('classes')->where('school_id', $school_id)->get()->getResultArray();
    $school_name = isset($student_list_school_name) ? (string) $student_list_school_name : '';
} else {
    // CI3-compatible fallback: compute data directly in the view.
    $where = array('school_id' => $school_id, 'session' => active_session());
    if (!empty($class_id) && $class_id != 'all') {
        $where['class_id'] = $class_id;
    }

    $enrols = db()->table('enrols')->select('DISTINCT(student_id), school_id, session')->where($where)->get()->getResultArray();
    $student_count = count($enrols);
    $students_data = array();
    $active_count = 0;

    foreach ($enrols as $enroll) {
        $student = db()->table('students')->where('id', $enroll['student_id'])->get()->getRowArray();
        if (!$student) {
            continue;
        }
...
if ($school_name === '') {
    $school_name = (string) db()->table('schools')->where('id', $school_id)->get()->getRow()->name ?? '';
}
?>
    ...
    <div id="exp-list-body">
        ...
        <div class="exp-list-item" 
            ...
            <div style="display: flex; gap: 0.5rem;">
                ...
                <a href="<?php echo site_url('app/member/edit/'.$student['id']); ?>" class="exp-action-btn" title="<?php echo get_phrase('edit'); ?>"
                   style="width: 32px; height: 32px; border-radius: 8px; border: none; background: linear-gradient(135deg, #eef2ff, #e0e7ff); color: var(--exp-primary); cursor: pointer; display: flex; align-items: center; justify-content: center;">
                ...
                <?php if($student['user_status'] == 1): ?>
                    <button type="button" class="exp-action-btn" title="<?php echo get_phrase('deactivate'); ?>"
                            onclick="confirmModal('<?php echo site_url('app/member/status/'.$student['id'].'/'.$student['user_id'].'/0'); ?>', showAllStudents)"
                ...
                <?php else: ?>
                    <button type="button" class="exp-action-btn" title="<?php echo get_phrase('activate'); ?>"
                            onclick="confirmModal('<?php echo site_url('app/member/status/'.$student['id'].'/'.$student['user_id'].'/1'); ?>', showAllStudents)"
                ...
                <button type="button" class="exp-action-btn" title="<?php echo get_phrase('delete'); ?>"
                        onclick="confirmModal('<?php echo site_url('app/member/delete/'.$student['id'].'/'.$student['user_id']); ?>', showAllStudents)"
                        style="width: 32px; height: 32px; border-radius: 8px; border: none; background: linear-gradient(135deg, #fef2f2, #fecaca); color: #dc2626; cursor: pointer; display: flex; align-items: center; justify-content: center;">
