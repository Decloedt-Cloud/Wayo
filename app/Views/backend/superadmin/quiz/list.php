<div class="row mb-3">
    ...
    <div class="col-md-4 toll-free-box text-center text-white pb-2" style="background-color: #6c757d; border-radius: 10px;">
        ...
        <span><?php echo get_phrase('class'); ?> : <?php echo db()->table('classes')->where('id', $class_id)->get()->getRow()->name ?? ''; ?></span><br>
        <span><?php echo get_phrase('cours'); ?> : <?php echo db()->table('course')->where('id', $cours_id)->get()->getRow()->title ?? ''; ?></span><br>
...
<?php  ?>
    <table class="table table-bordered table-responsive-sm" width="100%">
        ...
        <tbody>
        <?php 
        $enrols = db()->table('enrols')->where('class_id', $class_id)->get()->getResultArray();
        
        foreach ($enrols as $enrol):
            $student = db()->table('students')->where('id', $enrol['student_id'])->get()->getRowArray();
            if(!$student) continue;
            $quiz_lists = $this->crud_model->get_quiz($quiz_id ,$student['user_id'] );
                 ?>
