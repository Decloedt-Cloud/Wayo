...
$rooms = db()->table('rooms')->where('school_id', $school_id)->where('Etat', 1)->get()->getResultArray();
...
<div class="row mt-4">
    <?php foreach ($rooms as $room):
        $className = db()->table('classes')->where('id', $room['class_id'])->get()->getRow()->name ?? '';
        ...
        <div class="col-md-4">
            <div class="meeting-card position-relative">
                ...
                <div class="d-flex justify-content-between align-items-center mt-2 start-text">
                    ...
                    <!-- Bouton de copie du lien -->
                    <!-- <button onclick="rightModal('<?php echo site_url('modal/popup/class_room/edit/'.$room['id']); ?>', '<?php echo get_phrase('update_room'); ?>')" class="btn btn-outline-secondary " 
                           id="copy-btn-<?php echo $room['id']; ?>"
                            >
...
function closeModal() {
   ...
   $(document).ready(function () {
        ...
        $('#appointmentForm').on('submit', function (e) {
            ...
            if (Array.isArray(section)) { $sections = section.join(','); // Convertir ["1", "2", "3"] → "1,2,3"
            }
