...
<?php
  ...
  $school_data = is_object($settingsModel) ? ($settingsModel->get_current_school_data() ?? []) : [];
  $settings_school = is_object($settingsModel) ? ($settingsModel->get_current_settings_school_data() ?? []) : [];
  $result = $db->table('settings_school')->where('school_id', school_id())->get()->getRowArray();
  $result_defaults = ['vat' => 0, 'system_currency' => '', 'currency_position' => 'left'];
  $result = array_merge($result_defaults, is_array($result) ? $result : []);
  
  $billing_entities = [];
  $billingEntityService = service('billingEntityService');
  if (is_object($billingEntityService) && method_exists($billingEntityService, 'get_all_active')) {
    try {
      $billing_entities = $billingEntityService->get_all_active() ?? [];
    } catch (\Throwable $e) {
      $billing_entities = [];
    }
  }
  if (empty($billing_entities)) {
    try {
      $billing_entities = $db->table('billing_entities')
        ->where('is_active', 1)
...
<div class="row">
    ...
    <div class="main-card">
        ...
        <form method="POST" class="col-12 systempriceAjaxForm" action="<?php echo site_url('superadmin/payment_settings/price'); ?>" id = "price_settings">
  ...
  <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
    ...
    <div class="main-card">
        ...
        <form method="POST" class="col-12 systemvatAjaxForm," action="<?php echo site_url('superadmin/payment_settings/vat'); ?>" id = "vat_settings">
          <!-- Champ caché pour le jeton CSRF -->
           <input type="hidden" name="<?=csrf_token();?>" value="<?=csrf_hash();?>" />
    ...
    <div class="main-card">
        ...
        <form method="POST" class="col-12 systemAjaxForm" action="<?php echo site_url('superadmin/payment_settings/system'); ?>" id = "system_settings">
          <!-- Champ caché pour le jeton CSRF -->
           <input type="hidden" name="<?=csrf_token();?>" value="<?=csrf_hash();?>" />
              ...
              <div class="form-group row mb-3">
                ...
                <div class="col-md-9">
                  <select class="form-control"  id = "system_currency" name="system_currency" required>
                    ...
                    <?php
                    $currencies = is_object($settingsModel) ? ($settingsModel->get_currencies() ?? []) : [];
            ...
            $school_country = $db->table('schools')->where('id', school_id())->get()->getRow('country');
            ?>
...
<script>
  ...
  function checkPriceForParticulier(input) {
    const userType = <?php echo json_encode($settings_school['type'] ?? ''); ?>;
