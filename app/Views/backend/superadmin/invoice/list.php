<?php
/**
 * Superadmin Invoice List - Professional Edition
 * Optimized for use with index.php filters
 */

$invoices = $this->crud_model->get_invoice_by_date_range_superadmin($date_from, $date_to, $selected_class, $selected_status);

// Get school currency for costs display
$school_currency = db()->table('settings_school')->where('school_id', school_id())->get()->getRowArray()['system_currency'] ?? '' ?? 'EUR';
...
$expenses = $this->crud_model->get_expense($date_from, $date_to);
...
<div class="ai-table-container">
    ...
    <table class="ai-table" id="invoiceTable">
        <thead>
            <tr>
                <th>#</th>
                <th><?php echo get_phrase('community'); ?></th>
    ...
    <tbody>
            ...
            <?php foreach ($invoices as $inv):
                            ...
                            $student_entry = db()->table('students')->where('id', $inv['student_id'])->get()->getRowArray();
                            $student = [];
                            
                            if ($student_entry && isset($student_entry['user_id'])) {
                                $student = db()->table('users')->where('id', $student_entry['user_id'])->get()->getRowArray();
                            }
                            // 2. Si échec, essai direct sur la table users (cas School Join / Adhésion communauté où ID = User ID)
                            if (empty($student)) {
                                $student = db()->table('users')->where('id', $inv['student_id'])->get()->getRowArray();
                            }

                            // 3. Cas spécifique Subscription Admin (basé sur school_id) : Récupérer l'admin de l'école
                            if (empty($student) && isset($inv['payment_type']) && $inv['payment_type'] == 'subscription_admin') {
                                $student = $this->user_model->get_school_admin($inv['school_id']);
                            }

                            $class = isset($inv['class_id']) ? $this->crud_model->get_class_details_by_id($inv['class_id']) : null;
                            $community_details = $this->crud_model->get_community_details_by_id($inv['school_id']);
            ...
            <tr data-status="<?php echo $inv['status']; ?>" data-fx="<?php echo $fx ? '1' : '0'; ?>">
                ...
                <td>
                    <div style="font-weight: 600; color: var(--ai-dark); font-size: 0.85rem;">
                        <?php echo $community_details['name']; ?>
                    </div>
                </td>
                <td>
