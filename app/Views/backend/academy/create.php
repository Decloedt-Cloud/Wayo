                                ...
                                <div class="p-4 p-lg-5">
                                    ...
                                    <div class="quiz-form-group">
                                        ...
                                        if(session()->get('teacher_login') == 1) {
                                            ...
                                            // Resolve teacher id for active school first, then fallback to any teacher row for this user.
                                            $teacher_data = [];
                                            if ($current_school_id > 0) {
                                                $teacher_data = db()->table('teachers')->where('user_id', $user_id)->where('school_id', $current_school_id)->get()->getRowArray() ?? [];
                                            }
                                            if (empty($teacher_data)) {
                                                $teacher_data = db()->table('teachers')->where('user_id', $user_id)->get()->getRowArray() ?? [];
                                            }
                                            ...
                                            if ($teacher_id_perm) {
                                                ...
                                                // Fallback for migrated data: allow any class linked to this teacher if specific flags are missing.
                                                if (empty($permitted_class_ids)) {
                                                    $all_perm_rows = db()->table('teacher_permissions')->where('teacher_id', $teacher_id_perm)->get()->getResultArray();
