...
<?php
$academy_lms_model = (isset($this->lms_model) && is_object($this->lms_model))
    ? $this->lms_model
    : model('App\\Models\\addons\\Lms_model');

$to_rows = static function ($result): array {
    $normalize_rows = static function (array $rows): array {
        $normalized = [];
        foreach ($rows as $row) {
            if (is_array($row)) {
                $normalized[] = $row;
    ...
    if (is_object($result)) {
        ...
        if (method_exists($result, 'getResultArray')) {
            return $normalize_rows($result->getResultArray());
        }
        if (method_exists($result, 'getResult')) {
            $rows = $result->getResult();
            return is_array($rows) ? $normalize_rows($rows) : [];
        }
    }
    return [];
};
...
<div class="curriculum-container">
    ...
    <div class="editor-panel" id="editorPanel" style="display: none;">
        <div class="editor-card">
            ...
            <!-- CSRF Token -->
            <input type="hidden" id="csrf_name" value="<?php echo csrf_token(); ?>">
            ...
            <div class="outline-content" id="sectionsContainer">
                ...
                <div id="sectionsContent">
                    ...
                    foreach ($course_sections as $section): 
                        $section_counter++;
                        $lessons = $to_rows($academy_lms_model->get_lessons('section', $section['id']));
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                if (response.csrf) {
                    const csrfNameEl = document.getElementById('csrf_name');
                    const csrfHashEl = document.getElementById('csrf_hash');
                    if (csrfNameEl) csrfNameEl.value = response.csrf.csrfName || csrfName;
                    if (csrfHashEl) csrfHashEl.value = response.csrf.csrfHash || csrfHash;
                }
                ...
                if (!lessonId) {
                    ...
                    const currentLessonIdEl = document.getElementById('currentLessonId');
                    if (currentLessonIdEl) currentLessonIdEl.value = newLessonId;
                    const lessonEditorTitleEl = document.getElementById('lessonEditorTitle');
                    if (lessonEditorTitleEl) {
                        lessonEditorTitleEl.textContent = '<?php echo addslashes(get_phrase("edit_lesson")); ?>';
                    }
                    ...
                    const previewLessonIdEl = document.getElementById('previewLessonId');
                    const previewTitleEl = document.getElementById('previewTitle');
                    const previewLessonId = previewLessonIdEl ? previewLessonIdEl.value : null;
                    if (previewLessonId == lessonId && previewTitleEl) {
        ...
        error: function(xhr) {
            ...
            try {
                ...
                if (errorResponse.csrf) {
                    const csrfNameEl = document.getElementById('csrf_name');
                    const csrfHashEl = document.getElementById('csrf_hash');
                    if (csrfNameEl) csrfNameEl.value = errorResponse.csrf.csrfName || csrfName;
                    if (csrfHashEl) csrfHashEl.value = errorResponse.csrf.csrfHash || csrfHash;
                }
            } catch(e) {}
            if (!isAutosave) {
...
function generateOutlineSchemas() {
    ...
    const csrfName = document.getElementById('csrf_name')?.value || '<?php echo csrf_token(); ?>';
    const csrfHash = document.getElementById('csrf_hash')?.value || '<?php echo csrf_hash(); ?>';
