        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="header-title-wrapper">
                    ...
                    <?php
                    $course_id_safe = $course['id'] ?? 0;
                    $course_title_safe = $course['title'] ?? '';
                    $first_lesson_play_id = null;
                    if (is_array($first_lesson_id ?? null)) {
                        $first_lesson_play_id = $first_lesson_id['id'] ?? null;
                    } elseif (!empty($first_lesson_id)) {
                        $first_lesson_play_id = $first_lesson_id;
                    }
                    $play_lesson_url = $first_lesson_play_id
                        ? site_url('addons/lessons/play/' . slugify($course_title_safe) . '/' . $course_id_safe . '/' . $first_lesson_play_id)
                        : 'javascript:void(0)';
                    ...
                    <div class="header-container d-flex justify-content-between align-items-center">
                        <a href="<?= $play_lesson_url ?>"
                            class="btn btn-header btn-play <?= $first_lesson_play_id ? '' : 'disabled' ?>"
                            target="_blank">
                                            ...
                                            <div class="p-lg-5">
                                                ...
                                                <div class="quiz-form-group">
                                                    ...
                                                    if(session()->get('teacher_login') == 1) {
                                                        ...
                                                        $current_school_id = (int) (session()->get('active_school_id') ?? school_id());
                                                        $teacher_data = [];
                                                        ...
                                                        if ($teacher_id_perm) {
                                                            $permitted_classes_result = db()->table('teacher_permissions')->select('class_id')->where('teacher_id', $teacher_id_perm)->groupStart()->where('marks', 1)->orWhere('attendance', 1)->groupEnd()->get()->getResultArray();
                                                            $permitted_class_ids = array_column($permitted_classes_result, 'class_id');
                                                            if (empty($permitted_class_ids)) {
                                                                $all_perm_rows = $to_rows(db()->table('teacher_permissions')->where('teacher_id', $teacher_id_perm)->get()->getResultArray());
                                                                $permitted_class_ids = array_column($all_perm_rows, 'class_id');
                                                            }
                                                            $permitted_class_ids = array_values(array_unique(array_map('intval', $permitted_class_ids)));
                                                        }
                                                    }
                                                    ?>
                                                    ...
                                                    <select class="quiz-form-control quiz-select main-form-field" name="class_id[]" id="class_id_add_cours" multiple required>
                                                        ...
                                                        <?php foreach ($to_rows($classes) as $class): ?>
                                                            <?php 
                                                            if(session()->get('teacher_login') == 1 && !empty($permitted_class_ids) && !in_array((int) $class['id'], $permitted_class_ids, true)) {
                                                                continue;
                                                            }
                                                            ?>
                                                ...
                                                <?php else: ?>
                                                    <div class="quiz-form-group">
                                                        ...
                                                        <select class="quiz-form-control quiz-select main-form-field" name="user_id[]" id="user_id" multiple required>
                                                            ...
                                                            <?php foreach ($to_rows($all_teachers) as $teacher): ?>
