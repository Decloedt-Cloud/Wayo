		...
		<div class="card">
			<div class="card-body">
				<?php
					...
					$student_datas = db()->table('students')->where('user_id', $user_id)->get()->getResultArray();
					if($student_datas){
						$school_ids = array();		
						foreach ($student_datas as $student_data) {
							$school_ids[] = $student_data['school_id'];
						}
						$announcements = db()->table('announcement')->whereIn('school_id', $school_ids)->get()->getNumRows();
					} else {
						$announcements = 0;
					}
				...
				<?php if($announcements > 0): ?>
					<table id="basic-datatable" class="table table-striped dt-responsive nowrap" width="100%">
						...
						<tbody>
							<?php
							foreach($student_datas as $student_data){
								$enrols_datas = db()->table('enrols')->where('student_id', $student_data['id'])->where('school_id', $student_data['school_id'])->get()->getNumRows();
								$school_name = db()->table('schools')->where('id', $student_data['school_id'])->get()->getRow()->name ?? '';
								if($enrols_datas > 0){
								$announcements = db()->table('announcement')->where('school_id', $student_data['school_id'])->where('session', active_session())->get()->getResultArray();
