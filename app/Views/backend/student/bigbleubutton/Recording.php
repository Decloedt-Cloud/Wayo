                        ...
                        <select class="form-control"  name="school_id" id="school_id" onchange="schoolWiseClasse(this.value)">
                            <option value="<?php echo 'all'; ?>" <?php if($selected_school_id == 'all') echo 'selected'; ?>><?php echo get_phrase('all'); ?></option>
                                 <?php 
                                    ...
                                    $query = db()->table('schools')->select('*,schools.id as id')->from('schools')->join('students', 'schools.id = students.school_id', 'left')->where('students.user_id', $user_id)->get()->getResultArray();
                                ?>
                ...
                <tbody id="recordingTable">
                    <?php foreach ($appointments as $appointment): ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars(db()->table('classes')->where('id', $appointment['classe_id'])->get()->getRow()->name ?? '') ?></td>
                            <td>
                                <?php
                                    if (!empty($appointment['section'])) {
                                        ...
                                        foreach ($section_ids as $id) {
                                            $name = db()->table('sections')->where('id', $id)->get()->getRow()->name ?? '';
                            ...
                            <td>
                                <?= !empty($appointment['recordings']) && isset($appointment['recordings'][0]['duration']) 
