...
$student_data = db()->table('students')->where('user_id', $user_id)->get()->getResultArray();
$student_rows = [];
if (is_object($student_data)) {
    if (method_exists($student_data, 'result_array')) {
        $student_rows = $student_data;
    } elseif (method_exists($student_data, 'getResultArray')) {
        $student_rows = $student_data->getResultArray();
    }
} elseif (is_array($student_data)) {
    $student_rows = $student_data;
}

// Community count from user_schools where user is a member/student.
$communities_count = (int) $this->db->table('user_schools')
...
if (count($student_data) > 0) {
    $student_ids = array();
    $student_school_ids = array();
    foreach ($student_data as $student) {
        $student_ids[] = (int) $student['id'];
        if (isset($student['school_id'])) {
            $student_school_ids[] = (int) $student['school_id'];
        }
    }
    $student_ids = array_unique($student_ids); // Remove duplicates
    $student_school_ids = array_values(array_filter(array_unique($student_school_ids)));

    if (!empty($student_ids)) {
        $active_session_id = active_session();
        $enrol_builder = $this->db->table('enrols')
            ->select('class_id')
            ->whereIn('student_id', $student_ids);
        if (!empty($student_school_ids)) {
            $enrol_builder->whereIn('school_id', $student_school_ids);
        }
        if (!empty($active_session_id)) {
            $enrol_builder->where('session', $active_session_id);
        }
        $enrol_rows = $enrol_builder->get()->getResultArray();

        if (empty($enrol_rows) && !empty($active_session_id)) {
            // Fallback for migrated rows where session is missing/inconsistent.
            $enrol_fallback_builder = $this->db->table('enrols')
                ->select('class_id')
                ->whereIn('student_id', $student_ids);
            if (!empty($student_school_ids)) {
                $enrol_fallback_builder->whereIn('school_id', $student_school_ids);
            }
            $enrol_rows = $enrol_fallback_builder->get()->getResultArray();
        }

        if (!empty($enrol_rows)) {
            $class_ids = array();
            foreach ($enrol_rows as $enrol) {
                $cid = (int) ($enrol['class_id'] ?? 0);
                if ($cid > 0) {
                    $class_ids[] = $cid;
                }
            }
            ...
            if (!empty($class_ids)) {
                 $courses_builder = $this->db->table('course')
                    ->select('course.*')
                    ->join('course_classes', 'course.id = course_classes.course_id', 'inner')
                    ->whereIn('course_classes.class_id', $class_ids)
                    ->where('course.status', 'active')
                    ->groupBy('course.id');
                 $my_courses = $courses_builder->get()->getResultArray();
            }
        }
    }
}
                    ...
                    <div class="stat-value">
                        ...
                        if (count($student_data) > 0) {
                          ...
                          if(!empty($student_ids)){
                             ...
                             if (empty($enrol_rows) && !empty($active_session_id)) {
                               // Fallback for migrated legacy rows with missing session.
                               $enrol_rows = $this->db->table('enrols')
                                 ->select('class_id, school_id')
                                 ->whereIn('student_id', $student_ids)
                                 ->get()
                                 ->getResultArray();
                             ...
                             $unique_classes = [];
                             foreach ($enrol_rows as $row) {
                               $class_id = (int) ($row['class_id'] ?? 0);
                               if ($class_id <= 0) {
                                 continue;
                               }
                               $school_id = (int) ($row['school_id'] ?? 0);
                               $unique_classes[$school_id . '-' . $class_id] = true;
                             }
            ...
            <div class="modern-card fade-up delay-3" style="margin-top: 1.5rem;">
                ...
                <div class="modern-card-body p-0">
                    <?php foreach (array_slice($my_courses, 0, 5) as $course): ?>
                        ...
                        // Progress
                        $progress_value = function_exists('course_progress') ? (int) course_progress($course['id']) : 0;
                        
                        // First Lesson
                        $lms_model = $this->lms_model ?? model('App\\Models\\addons\\Lms_model');
                        $lessons = $lms_model ? $lms_model->get_lessons('course', $course['id']) : [];
                        $lessons_rows = [];
                        if (is_array($lessons)) {
                            $lessons_rows = $lessons;
                        } elseif (is_object($lessons)) {
                        ...
                        <div class="course-list-item">
                            ...
                            <div class="course-action">
                                <?php if (count($lessons_rows) > 0 && !empty($first_lesson)): ?>
            ...
            <div class="modern-card fade-up delay-3">
              ...
              <div class="modern-card-body">
                <div class="events-list">
                  ...
                  $events = $this->crud_model->get_current_month_events();
                  $has_events = false;
