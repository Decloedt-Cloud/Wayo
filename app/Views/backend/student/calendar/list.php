...
const CalendarApp = {
  ...
  communityRecoveryAttempted: false,
  communityRecoveryInProgress: false,
  lastEventId: null,
  lastMeetingId: null,
  ...
  loadEvents(start, end, successCallback, failureCallback, nocache = false) {
    ...
    $.ajax({
        url: '<?php echo site_url('app/get_events'); ?>',
        type: 'GET',
        data: data,
        dataType: 'json',
        ...
        success: (response) => {
            try {
                ...
                if (data.status === 'success') {
                    ...
                    if (errorMessage.includes('Not authorized for this school')) {
                        errorMessage = '<?php echo get_phrase("⚠️ Community access has changed Please refresh the page and select your active community again."); ?>';
                        if (!this.calendarAccessDenied) {
                            this.showNotification('warning', errorMessage);
                        }
                        this.calendarAccessDenied = true;
                        this.stopPolling();
                        this.stopActiveMeetingsPolling();
                        successCallback([]);
                        return;
                ...
                const fallbackMessage = '<?php echo get_phrase("⚠️ Your calendar is empty To see events, join a class or wait for your registration to be validated."); ?>';
                this.showNotification('error', fallbackMessage);
                successCallback([]);
            }
        },
        error: (xhr) => {
        ...
        success: (response) => {
            try {
                ...
                if (data.status === 'success' && data.data && data.data.length > 0) {
                    ...
                    $.ajax({
                        url: '<?php echo site_url('app/get_school_data'); ?>',
                        type: 'POST',
                        data: {
                            school_id: event.school_id,
                            [csrfName]: csrfHash
            ...
            try {
                ...
                if (data.status === 'success' && data.data && data.data.length > 0) {
                    ...
                    if (buttonText === startMeetingPhrase) {
                        $.ajax({
                            url: '<?php echo site_url('app/start_meeting'); ?>',
                            type: 'POST',
