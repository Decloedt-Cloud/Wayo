...
window.confirmModal = function(delete_url, callback, options) {
    options = options || {};
    var modalTitle = options.title || '<?php echo get_phrase("are_you_sure"); ?>';
    var modalText = options.text || '<?php echo get_phrase("you_will_not_be_able_to_revert_this"); ?>';
    var modalIcon = options.icon || 'warning';
    var confirmText = options.confirmButtonText || '<?php echo get_phrase("yes_delete_it"); ?>';
    var cancelText = options.cancelButtonText || '<?php echo get_phrase("cancel"); ?>';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            // Always use the hidden form token as single source of truth
            // to avoid stale duplicate inputs elsewhere in DOM.
            ...
            $.ajax({
                url: delete_url,
                type: 'POST',
                data: $.extend({}, (options && options.requestData) ? options.requestData : {}, { [csrfName]: csrfHash }),
                headers: { 'X-CSRF-TOKEN': csrfHash },
                dataType: 'json',
                success: function(response) {
                    // Support both CSRF payload shapes used across controllers:
                    // {csrf:{csrfName,csrfHash}} and {csrf:{name,hash}}.
                    if (response && response.csrf) {
                        var returnedCsrfName = response.csrf.csrfName || response.csrf.name || csrfName;
                        var returnedCsrfHash = response.csrf.csrfHash || response.csrf.hash || '';
                        if (returnedCsrfHash) {
                            window.__csrfName = returnedCsrfName;
                            window.__csrfHash = returnedCsrfHash;
                            var csrfInput = $('input[name="' + returnedCsrfName + '"]');
                            if (csrfInput.length) {
                                csrfInput.val(returnedCsrfHash);
                            } else {
                                // Ensure token exists even on pages without the hidden form input rendered yet.
                                $('#delete_form').append('<input type="hidden" name="' + returnedCsrfName + '" value="' + returnedCsrfHash + '" />');
                            }
                        }
                    }
                    ...
                    if (response.status) {
                        ...
                        var fallbackError = (options && options.errorNotificationText)
                            ? options.errorNotificationText
                            : '<?php echo js_phrase('failed_to_delete'); ?>';
                        showNotification('error', response.notification || fallbackError);
                    }
                },
                error: function(xhr, status, error) {
                    var fallbackError = (options && options.errorNotificationText)
                        ? options.errorNotificationText
                        : '<?php echo js_phrase('failed_to_delete'); ?>';
                    showNotification('error', fallbackError);
                }
            });
        }
    });
