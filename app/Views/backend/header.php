...
if ($active_school_id && $user_id) {
    $user_school_check = $this->db->table('user_schools')->where('user_id', $user_id)->where('school_id', $active_school_id)->get()->getRow();
...
if ($active_school_id && $user_belongs_to_school) {
    $school = $this->db->table('schools')->select('name as community_name')->where('id', $active_school_id)->get()->getRow();
    if ($school) {
        $current_name = $school->community_name;
    }
    // Récupère le rôle dans user_schools
    $user_school = $this->db->table('user_schools')->select('role')->where('user_id', $user_id)->where('school_id', $active_school_id)->get()->getRow();
    ...
    if ($active_school_id && !$user_belongs_to_school && strtolower($active_role) === 'student') {
        session()->set('active_school_id', null);
        session()->set('school_id', null);
        $active_school_id = null;
    }
}
?>
<?php echo view('create_community_modal.php'); ?>
...
<div class="navbar-custom topnav-navbar navbar-color">
    <div class="container-fluid">
            ...
            <li class="dropdown notification-list profile-menu">
                <a class="nav-link dropdown-toggle nav-user  user-dropdown arrow-none me-0" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="false"
                    ...
                    <span class="account-user-avatar">
                        <img src="<?php echo $this->user_model->get_user_image(session()->get('user_id')); ?>" alt="user-image" class="rounded-circle user-style">
                    </span>
                    <span>
                        <span class="account-user-name"><?php echo $user_name; ?></span>
                        <?php 
                            $role_raw = $this->db->table('users')->where('id', $user_id)->get()->getRow('role');
                            $user_role = strtolower((string) ($role_raw ?? ''));
                ...
                <div class="dropdown-menu dropdown-menu-end dropdown-menu-animated topbar-dropdown-menu profile-dropdown dropup-mobile">
                    ...
                    <?php if (session()->get('user_type') == 'superadmin' || session()->get('user_type') == 'admin'): ?>
                        <!-- item-->
        ...
        <!-- LOGO -->
        <a href="<?php echo site_url(session()->get('role') . '/dashboard'); ?>" class="topnav-logo" style="min-width: unset;">
        ...
        <div class="app-search d-flex align-items-center flex-wrap gap-3 pt-1 mt-2">
                ...
                if (!empty($user_schools)) {
                    foreach ($user_schools as $user_school) {
                        ...
                        if ($role_in_school === 'admin' || $role_in_school === 'teacher') {
                            ...
                            $student_approved = db()->table('students')
                                ->where('user_id', $user_id)
            ...
            ?>
            <?php if (strtolower((string) (session()->get('role') ?? '')) !== 'superadmin' && $user_has_community): ?>
...
<script type="text/javascript">
    ...
    function getCsrfHashFromCookie() {
        const escapedName = csrfCookieName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const match = document.cookie.match(new RegExp('(?:^|; )' + escapedName + '=([^;]*)'));
        return match && match[1] ? decodeURIComponent(match[1]) : '';
    }

    function getCsrfInputElement() {
        return document.querySelector('input[name="<?= csrf_token(); ?>"]')
            || document.getElementById('community_csrf_token')
    ...
    $.ajaxSetup({
        ...
        beforeSend: function(xhr, settings) {
            ...
            const tokenName = Object.keys(csrfData)[0];
            const tokenValue = tokenName ? csrfData[tokenName] : null;
            if (!tokenName || !tokenValue) return;

            // FormData: append token safely.
            ...
            // String payload: append query style token.
            if (typeof settings.data === 'string') {
                settings.data += (settings.data ? '&' : '') + encodeURIComponent(tokenName) + '=' + encodeURIComponent(tokenValue);
                return;
            }

            // Object payload (default in many $.ajax calls).
            if (settings.data && typeof settings.data === 'object') {
                if (!(tokenName in settings.data)) {
                    settings.data[tokenName] = tokenValue;
                }
                return;
            }

            // Empty payload: initialize with token.
            settings.data = { [tokenName]: tokenValue };
        }
    });

    function getCsrfData() {
        const cookieHash = getCsrfHashFromCookie();
        if (csrfName && cookieHash) {
            return { [csrfName]: cookieHash };
        }

        const csrfInput = getCsrfInputElement();
        if (csrfInput && csrfInput.name && csrfInput.value) {
            return { [csrfInput.name]: csrfInput.value };
        }
    ...
    function updateCsrfHash(newHash) {
        if (newHash) {
            csrfHash = newHash;
            const currentName = csrfName || '<?= csrf_token(); ?>';
            document.querySelectorAll('input[name="' + currentName + '"]').forEach(function(input) {
                input.value = newHash;
            });
            const fallbackInput = getCsrfInputElement();
            if (fallbackInput) {
                fallbackInput.value = newHash;
                csrfName = fallbackInput.name || csrfName;
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            success: function(response) {
                ...
                if (typeof response === 'string') {
                    try {
                        ...
                        console.error('Invalid JSON response from get_user_communities:', response);
                        listContainer.innerHTML = '<div class="p-3 text-center text-danger">' + TRANSLATIONS.invalidResponse + '</div>';
                        return;
                    }
                }
                const communityData = Array.isArray(res?.data) ? res.data : Object.values(res?.data || {});
                if (res.status === 'success' && communityData.length > 0) {
                    // Stocker toutes les communautés pour la recherche
    ...
    function renderCommunitiesList(communities, searchTerm = '') {
        ...
        const normalizeRole = (role) => {
            ...
            if (r.includes('teacher') || r.includes('mentor')) return 'teacher';
            if (r.includes('student') || r.includes('member')) return 'student';
        ...
        const validRoles = ['admin', 'teacher', 'superadmin', 'student', 'parent', 'accountant', 'librarian', 'driver'];
        let filtered = normalizedCommunities.filter(c => validRoles.includes(c.role));
        if (filtered.length === 0 && normalizedCommunities.length > 0) {
            // If role normalization still fails for some legacy records, keep raw list to avoid false "No community".
            filtered = normalizedCommunities;
        }
        
        // Filtrer par terme de recherche
        const searchFiltered = searchTerm 
            ? filtered.filter(c => String(c.community_name || '').toLowerCase().includes(searchTerm.toLowerCase()))
            : filtered;
