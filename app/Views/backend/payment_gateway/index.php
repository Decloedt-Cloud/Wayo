...
<html lang="en">
	<head>
	    ...
	    <title><?php echo get_phrase($page_title); ?><?php 
            $school_id = school_id();
            if ($school_id > 0) {
                $dbTitle = \Config\Database::connect();
                $schoolNameForTitle = (string) ($dbTitle->table('schools')->where('id', $school_id)->get()->getRow('name') ?? '');
                echo ' | ' . $schoolNameForTitle;
            }
        ?></title>
	...
	<body>
		...
		// Ensure role flags are always available across all template branches.
		...
		$school_data = $db->table('schools')->where('id', $school_id_for_vat)->get()->getRowArray();
		...
		$moroccoService = service('moroccoB2BService');
		...
		if (!$billingEntityService) {
			$billing_entity = null;
		} else {
			$billing_entity = $billingEntityService->get_entity_for_tax_residence($tax_residence);
		}
		...
		if ($entity_id) {
			...
			$entity_methods = $paymentMethodModel->get_for_entity($entity_id, true);
		...
		if ($is_subscription_admin && $entity_id) {
			...
			$stripe_creds = $billingEntityCredentialsModel->get_stripe_credentials($entity_id);
			...
			$paypal_creds = $billingEntityCredentialsModel->get_paypal_credentials($entity_id);
		...
		if ($invoice_details['payment_type'] === 'subscription_admin' && $tax_residence === 'MA') {
			...
			$is_morocco_b2b = $moroccoService->shouldApplyMoroccoB2BRules($morocco_context);
		}

		// Apply Morocco B2B calculations for subscription invoices
		if ($is_morocco_b2b) {
			// Calculate Morocco B2B values
			$morocco_calculations = $moroccoService->calculateMoroccoB2BAmounts($invoice_details['total_amount']);
		...
		if (!$is_morocco_b2b && $vat_applicable && !empty($tax_residence) && in_array($tax_residence, ['MA', 'AE', 'UAE'])) {
			$vatResolver = new \App\Libraries\VatResolver();

			try {
				// Calculate VAT using VatResolver (toujours en MAD d'abord)
				$vat_calculation = $vatResolver->calculateVatBreakdown((float)$amount_to_pay, $tax_residence);
				...
				if ($tax_residence === 'UAE' || $tax_residence === 'AE') {
                    ...
                    if ($invoice_currency === 'AED') {
                        ...
                        $fxratesService = new \App\Libraries\FxRatesService();
                        
                        // Utiliser la méthode convert pour obtenir le taux
                        $test_convert = $fxratesService->convert(1, 'MAD', 'AED');
						...
						action="<?php echo $stripe_action_url; ?>">
											...
											<?php endif; ?>
													...
													<div class="info-box">
														<?php 
															if (!empty($user_details['email'])) {
																...
																$stu = $db->table('students')->where(['id' => $stu_id])->get()->getRowArray();
																if ($stu) {
																	$usr = $db->table('users')->where(['id' => $stu['user_id']])->get()->getRowArray();
																	$usr_email = $usr ? $usr['email'] : 'N/A';
																} else {
																	// 2. Fallback: Check if $stu_id is actually a user_id (Data Inconsistency Fix)
																	$usr_direct = $db->table('users')->where(['id' => $stu_id])->get()->getRowArray();
																	if ($usr_direct) {
												...
												<div class="package-details mt-3">
													<strong><?php echo get_phrase('Member_name');?> | <?php echo htmlspecialchars((string)($user_details['name'] ?? 'N/A'));?></strong>
												</div>
