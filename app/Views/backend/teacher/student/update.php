...
$selected_class_ids = isset($modal_student_selected_class_ids) && is_array($modal_student_selected_class_ids) ? $modal_student_selected_class_ids : [];
$classes = isset($modal_student_classes) && is_array($modal_student_classes) ? $modal_student_classes : [];
$student_id = (int) ($student_id ?? 0);

// Fallback for full-page edit route (teacher/student/edit/{id}) when modal_* payload is not set.
if (empty($student) && $student_id > 0) {
    $studentQuery = $this->db->table('students')->where('id', $student_id);
    if ((int) $school_id > 0) {
        $studentQuery->where('school_id', (int) $school_id);
    }
    $student = $studentQuery->get()->getRowArray() ?? [];
    if (!empty($student)) {
        $enrolRows = $this->db->table('enrols')
            ->where('student_id', $student_id)
            ->get()
            ->getResultArray();
        $selected_class_ids = array_values(array_unique(array_map('intval', array_column($enrolRows, 'class_id'))));

        $target_school_id = (int) ($student['school_id'] ?? 0);
...
$modal_student_not_found = isset($modal_student_not_found) ? (bool) $modal_student_not_found : empty($student);
?>
...
<div class="col-xl-12">
   ...
   <div class="row">
        <div class="col-12">
            ...
            <div class="main-card">
                <div class="card-body">
                <?php if($modal_student_not_found): ?>
                    <div class="alert alert-danger"><?php echo get_phrase('student_not_found'); ?></div>
                    <a href="<?php echo site_url('teacher/student'); ?>" class="btn btn-primary"><?php echo get_phrase('go_back'); ?></a>
                    <?php return; ?>
                <?php endif; ?>
                <h4 class="text-center mx-0 py-2 mt-0 mb-3 px-0 text-white bg-primary"><?php echo get_phrase('update_student_information'); ?></h4>
                <form method="POST" class="col-12 d-block ajaxForm" action="<?php echo site_url('teacher/student/updated/' . $student_id . '/' . $student['user_id']); ?>" id="student_update_form" enctype="multipart/form-data">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group ">
                            ...
                            <select name="class_id[]" id="class_id" class=" form-control" multiple="multiple" required data-live-search="true">
                                ...
                                foreach ($classes as $class) { ?>
                                    <option value="<?php echo $class['id']; ?>" data-class-name="<?php echo $class['name']; ?>" <?php if (in_array((int) $class['id'], $selected_class_ids)) echo 'selected'; ?>><?php echo $class['name']; ?></option>
                                <?php } ?>
                        ...
                        <div class="form-group ">
                            ...
                            <div class=" position-relative" id="datepicker4">
                                <!-- <input type="text" class="form-control" data-provide="datepicker"  placeholder="<?php //echo get_phrase('birthday'); 
                                                                                                                        ?>" data-date-autoclose="true" data-date-container="#datepicker4" name = "birthday"   value="<?php //if($this->user_model->get_user_details($student['user_id'], 'birthday') != "") echo date('m/d/Y', strtotime($this->user_model->get_user_details($student['user_id'], 'birthday'))); 
                                                                                                                                                                                                                    ?>" > -->
                                <?php
