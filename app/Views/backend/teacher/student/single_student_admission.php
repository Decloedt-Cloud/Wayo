...
<?php
$classes = [];
if (isset($student_create_classes)) {
    if (is_array($student_create_classes)) {
        $classes = $student_create_classes;
    } elseif (is_object($student_create_classes)) {
        if (method_exists($student_create_classes, 'getResultArray')) {
            $classes = $student_create_classes->getResultArray();
        } elseif (method_exists($student_create_classes, 'result_array')) {
            $classes = $student_create_classes;
        }
    }
}
$student_user_id = isset($student['id']) ? (int) $student['id'] : 0;
?>

<form method="POST" class="col-12 d-block ajaxForm" action="<?php echo site_url('teacher/student/create_single_student/submit'); ?>" id = "student_admission_form" enctype="multipart/form-data">
        ...
        <div class="form-group row mb-3">
            ...
            <div class="col-md-9">
                <select name="class_id" id="class_id" class="form-control"  required >
                    ...
                    <?php foreach($classes as $class){ $class = is_object($class) ? (array) $class : $class; ?>
...
// Fonction pour récupérer et retourner le token CSRF
 ...
 $(".ajaxForm").submit(function(e) {
  ...
  function check() {
    ...
    var address = $("#example-textarea").val();
    var phone = $("#phone").val();
