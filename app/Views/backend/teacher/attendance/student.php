...
<?php $check_permission = isset($check_permission) ? (int) $check_permission : (int) has_permission($class_id, 'attendance'); ?>
<?php if ($check_permission): ?>
    <?php $school_id = (int) ($attendance_school_id ?? school_id()); ?>

    <!-- Hidden CSRF Token for AJAX handling -->
    <input type="hidden" class="fresh-csrf-token" name="<?=csrf_token();?>" value="<?=csrf_hash();?>" />
