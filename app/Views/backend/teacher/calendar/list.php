            ...
            <div class="modal-content exp-modal-content">
                ...
                <div class="modal-body modal-body-calendar">
                    <form id="recurrenceForm">
                        <div class="exp-form-group">
                            <label for="recurrenceTypePopup" class="exp-form-label"><span class="mdi mdi-calendar-sync"></span> <?php echo get_phrase('recurrence_type'); ?></label>
...
const enableToasts = <?php echo json_encode((bool) (isset($enable_toasts) ? $enable_toasts : true)); ?>;
...
const CalendarApp = {
  ...
  parseServerResponse(response) {
    ...
    try {
      ...
      console.error('Calendar response parse failed:', error, text);
  ...
  return this.checkAndRestoreActiveMeetings().then(() => {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
      ...
      this.checkAndRestoreActiveMeetings().then(() => {
        this.calendar.render();
        if (document.visibilityState === 'visible') {
          this.pollActiveMeetings();
        }
...
setupResizeListener() {
  ...
  loadEvents(start, end, successCallback, failureCallback) {
    ...
    // Clé de cache basée sur la plage de dates
    ...
    $.ajax({
            url: '<?php echo site_url('app/calendar/get_events'); ?>',
            type: 'GET',
            data: {
            ...
            success: (response) => {
                ...
                try {
                    const data = this.parseServerResponse(response);
    ...
    if (event.recurrence_type !== 'does_not_repeat' && event.occurrences && Object.keys(event.occurrences).length > 0) {
        ...
        if (!uniqueEventIds.has(uniqueId) && event.starting_date >= start.split('T')[0] && event.starting_date <= end.split('T')[0]) {
            ...
            uniqueEventIds.add(uniqueId);
                    ...
                    failureCallback(new Error('Failed to parse events'));
                }
            },
            error: (xhr) => {
                console.error('AJAX error:', xhr.status, xhr.statusText);
                this.showNotification('error', xhr.status === 403 ? 'Access denied' : 'Failed to load events');
                failureCallback(new Error(xhr.status === 403 ? 'Access denied' : 'Failed to load events'));
                },
            complete: () => {
                this.isLoading = false;
                $('#calendar').removeClass('loading');
  ...
  loadClassesWithEvents(start, end) {
    $.ajax({
      url: '<?php echo site_url('app/calendar/get_user_school'); ?>',
      type: 'GET',
      ...
      success: (response) => {
          try {
              ...
              if (data.status === 'success') {
                ...
                this.showNotification('error', data?.message || 'Failed to load school');
              }
          } catch (e) {
    ...
    $.ajax({
            ...
            success: (response) => {
                ...
                if (data.status === 'success' && data.data && data.data.length > 0) {
                  ...
                  $.ajax({
                        url: '<?php echo site_url('app/calendar/get_school_data'); ?>',
                        type: 'POST',
                        data: {
                            school_id: event.school_id,
                            [csrfName]: csrfHash
...
stopPolling() {
  ...
  validateForm(formId) {
    ...
    if (participantsField.length) {
      ...
      try {
        const parsed = JSON.parse(participantsField.val() || '[]');
      ...
      if (participantCount === 0) {
        this.showNotification('error', '<?php echo get_phrase("please_select_at_least_one_participant"); ?>');
        isValid = false;
      }
    }
    return isValid;
  ...
  bindGlobalEvents() {
    $('#createEventModal').on('show.bs.modal', () => {
      ...
      $('#participantsDropdown').html('<div class="pp-empty"><?php echo get_phrase("Write_something_to_search..."); ?></div>');
      $('#participantsInput').val('[]');
      this.resetPicker('create');
    ...
    $('#createEventForm').on('submit', (e) => {
      ...
      $.ajax({
        url: '<?php echo site_url('app/calendar/create'); ?>',
        type: 'POST',
        data: formData,
        success: (response) => {
            const data = this.parseServerResponse(response);
            if (data.status === 'success') {
              ...
              this.showNotification('success', data?.message || 'Event created successfully');
              this.clearEventCache();
              this.calendar.refetchEvents();
              this.calendar.render();
            } else {
              this.showNotification('error', data?.message || 'Failed to create event');
            }
            csrfHash = data?.csrf?.csrfHash || csrfHash;
        },
  ...
  $.ajax({
    url: '<?php echo site_url('app/calendar/update'); ?>',
    type: 'POST',
    data: formData,
    success: (response) => {
        const data = this.parseServerResponse(response);
        if (data.status === 'success') {
          ...
          this.resetRecurrenceModal();
          this.showNotification('success', data?.message || 'Event successfully updated');
          this.clearEventCache();
          this.calendar.refetchEvents();
          this.calendar.render();
        } else {
          this.showNotification('error', data?.message || 'Failed to update event');
        }
        csrfHash = data?.csrf?.csrfHash || csrfHash;
    },
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.ajax({
        url: '<?php echo site_url('app/calendar/delete'); ?>',
        type: 'POST',
        ...
        success: (response) => {
            ...
            if (data.status === 'success') {
              $('#eventEditModal').modal('hide');
              this.showNotification('success', data?.message || 'Event successfully deleted');
              this.clearEventCache();
              this.calendar.refetchEvents();
              this.calendar.render();
              csrfHash = data?.csrf?.csrfHash || csrfHash;
            } else {
              this.showNotification('error', data?.message || 'Failed to delete event');
            }
        },
        error: (xhr) => {
            ...
            try {
                ...
                if (data.status === 'success' && data.data && data.data.length > 0) {
                    ...
                    if (buttonText === '<?php echo get_phrase('Start Meeting'); ?>') {
                        $.ajax({
                            url: '<?php echo site_url('app/calendar/start_meeting'); ?>',
                            type: 'POST',
...
checkAndRestoreActiveMeetings() {
  return new Promise((resolve, reject) => {
    ...
    $.ajax({
      ...
      error: (xhr) => {
        console.error('checkAndRestoreActiveMeetings - AJAX error:', xhr);
        reject(xhr);
      }
    });
  });
