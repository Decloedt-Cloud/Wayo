...
if (isset($class_id)):
    ...
    $total_pages = (int) ceil($total_syllabuses / $per_page);
?>
...
<div class="syllabus-info-bar" role="status" aria-live="polite">
    <div class="syllabus-info-chip">
        ...
        <strong><?php echo htmlspecialchars((string) $class_name, ENT_QUOTES); ?></strong>
    </div>
</div>
...
<?php if($total_syllabuses > 0): ?>
    ...
    <div class="syllabus-list-item" role="row">
        ...
        <div role="gridcell">
            <a href="<?php echo site_url('app/syllabus/download/'.$syllabus['id']); ?>" class="syllabus-download-btn" download aria-label="<?php echo get_phrase('download_file') . ': ' . htmlspecialchars($syllabus['title'], ENT_QUOTES); ?>">
                <i class="mdi mdi-download" aria-hidden="true"></i> <span class="sr-only"><?php echo get_phrase('download'); ?></span>
            </a>
        </div>
        
        <div role="gridcell">
            <button type="button" class="syllabus-action-btn" 
                    onclick="confirmModal('<?php echo site_url('app/syllabus/delete/'.$syllabus['id']); ?>', showAllSyllabuses)"
    ...
    <nav class="syllabus-pagination" role="navigation" aria-label="<?php echo get_phrase('pagination'); ?>">
        ...
        <div class="pagination-controls">
            <button type="button" class="pagination-btn" onclick="changePage(<?php echo max(1, $current_page - 1); ?>)" <?php echo $current_page <= 1 ? 'disabled' : ''; ?> aria-label="<?php echo get_phrase('previous_page'); ?>" <?php echo $current_page <= 1 ? 'aria-disabled="true"' : ''; ?>>
            ...
            <button type="button" class="pagination-btn" onclick="changePage(<?php echo min($total_pages, $current_page + 1); ?>)" <?php echo $current_page >= $total_pages ? 'disabled' : ''; ?> aria-label="<?php echo get_phrase('next_page'); ?>" <?php echo $current_page >= $total_pages ? 'aria-disabled="true"' : ''; ?>>
...
function changePage(page) {
    ...
    $.ajax({
        url: '<?php echo site_url('app/syllabus/list/'); ?>' + class_id + '/' + page,
