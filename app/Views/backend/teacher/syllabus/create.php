...
<div class="exp-form-container">
    ...
    <form method="POST" class="d-block ajaxForm" action="<?php echo site_url('app/syllabus/create'); ?>" enctype="multipart/form-data">
...
$('document').ready(function(){
    ...
    $(".ajaxForm").submit(function(e) {
        ...
        var liveCsrfHash = getLiveCsrfHash();
        if (liveCsrfHash) {
            formData.set('<?php echo csrf_token(); ?>', liveCsrfHash);
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status) {
                    ...
                    var nextCsrfName = (response.csrf && (response.csrf.name || response.csrf.csrfName)) || '<?php echo csrf_token(); ?>';
                    var nextCsrfHash = (response.csrf && (response.csrf.hash || response.csrf.csrfHash)) || '';
                    if (nextCsrfHash) {
    ...
    function getLiveCsrfHash() {
        ...
        return $('input[name="<?php echo csrf_token(); ?>"]').val() || '';
    }
});
