...
// Récupérer les classes autorisées pour cet enseignant.
// Le schéma actuel de teacher_permissions ne contient pas de colonne 'syllabus';
// on utilise la même portée de classes que les autres modules teacher.
$permitted_class_ids = [];
if ($teacher_id) {
...
<section class="syllabus-filter-section" aria-labelledby="filter-heading">
    <div class="syllabus-filter-grid">
        <div class="syllabus-filter-group">
            ...
            <select name="class" id="class_id_syllabus" class="syllabus-filter-input" required aria-describedby="class-help">
                ...
                $enrols = !empty($class_ids) ? db()->table('enrols')
                ...
                foreach ($enrols as $enrol) {
                    ...
                    if (!isset($student_counts[$cid])) {
                        $student_counts[$cid] = 0;
                    }
                    $student_counts[$cid]++;
                }
...
<main class="syllabus-content-card" role="main" aria-labelledby="syllabus-list-heading">
    ...
    <div class="syllabus-content-body syllabus_content" role="region" aria-live="polite" aria-busy="false">
...
<script>
    ...
    var showAllSyllabuses = function(class_id) {
        ...
        $.ajax({
            url: '<?php echo site_url('app/syllabus/list/'); ?>' + class_id,
