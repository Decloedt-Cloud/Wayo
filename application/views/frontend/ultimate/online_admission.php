...
<script>
  (function() {
    ...
    function validateStep(stepIndex) {
      ...
      if (pane.dataset.step == "1" || pane.dataset.step == "2") {
        const hasDuplicate = document.querySelectorAll('[data-duplicate="true"]').length > 0;
        if (hasDuplicate) {
      ...
      if (pane.dataset.step == "1") {
        ...
        if (!name.value.trim() || name.value.trim().length < 2) ok = setInvalid(name, 'Nom obligatoire (2 caractères min)') && ok;
        else clearInvalid(name);
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value.trim())) ok = setInvalid(email, 'Email invalide') && ok;
        else clearInvalid(email);
        if (((phone.value) || '').replace(/\D/g, '').length < 8) ok = setInvalid(phone, 'Téléphone obligatoire (au moins 8 chiffres)') && ok;
        else clearInvalid(phone);
        if ((pass.value || '').length < 6) ok = setInvalid(pass, '6 caractères minimum') && ok;
        else clearInvalid(pass);
        if (pass.value !== pass2.value || (pass2.value || '').length < 6) ok = setInvalid(pass2, 'Les mots de passe ne correspondent pas') && ok;
      ...
      if (pane.dataset.step == "2") {
        ...
        if (!name.value.trim() || name.value.trim().length < 2) ok = setInvalid(name) && ok;
        else clearInvalid(name);
        if (!desc.value.trim() || desc.value.trim().length < 40) ok = setInvalid(desc) && ok;
        else clearInvalid(desc);
        if (!cat.value) ok = setInvalid(cat) && ok;
        else clearInvalid(cat);
        if (!Tax_residence.value) ok = setInvalid(Tax_residence) && ok;
        else clearInvalid(Tax_residence);
        if (!lang.value) ok = setInvalid(lang) && ok;
        else clearInvalid(lang);

        if (!street.value.trim() || street.value.trim().length < 3) ok = setInvalid(street) && ok;
        else clearInvalid(street);
        if (!/^[0-9]+$/.test(number.value.trim())) ok = setInvalid(number) && ok;
        else clearInvalid(number);
        if (!city.value.trim() || city.value.trim().length < 2) ok = setInvalid(city) && ok;
        else clearInvalid(city);
        if (!i_am.value.trim()) ok = setInvalid(i_am) && ok;
        else clearInvalid(i_am);
        if (!/^[0-9]{4,5}$/.test(postalCode.value.trim())) ok = setInvalid(postalCode) && ok;
        else clearInvalid(postalCode);
      }
