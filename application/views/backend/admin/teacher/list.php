...
foreach($teachers as $teacher){
    $user = $this->db->get_where('users', array('id' => $teacher['user_id']))->row_array();
    if(!$user) continue;
        ...
        <div class="exp-list-item" 
            ...
            <div style="display: flex; gap: 0.5rem;">
                <button type="button" class="exp-action-btn" title="<?php echo get_phrase('profile'); ?>"
                        onclick="largeModal('<?php echo site_url('modal/popup/teacher/profile/'.$teacher['id'])?>', '<?php echo $this->db->get_where('schools', array('id' => $school_id))->row('name'); ?>')"
