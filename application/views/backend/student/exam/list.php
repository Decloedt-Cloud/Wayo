...
if (!empty($enrolled_class_ids)) {
    ...
    // If student is not enrolled in any classes, return 0
    $this->db->where('1', '0');
}
...
$this->db->select('exams.*, classes.name as class_name, 
    (SELECT COUNT(*) FROM exam_responses WHERE exam_responses.exam_id = exams.id AND exam_responses.user_id = ' . $this->db->escape($user_id) . ') as already_passed');
$this->db->from('exams');
...
$this->db->limit($exams_per_page, 0);
        ...
        <div class="filter-row">
            ...
            <div class="filter-group">
                ...
                <select name="class_id" id="class_id" class="form-control filter-select">
                    ...
                    if (!empty($enrolled_class_ids)) {
                        $this->db->where_in('id', $enrolled_class_ids);
                    } else {
                        // If no enrolled classes, show none
                        $this->db->where('1', '0');
                    }
                    $this->db->where('school_id', $school_id);
                ...
                <div class="exam-item" data-exam-id="<?php echo $exam['id']; ?>">
                    <div class="exam-info">
                        ...
                        <div class="exam-details">
                            <div class="exam-name"><?php echo html_escape($exam['name']); ?></div>
                            <div class="exam-meta">
                                <span class="exam-date">
                                    <i class="fas fa-calendar"></i> <?php echo date('D, d-M-Y H:i', $exam['starting_date']); ?>
                                </span>
                            ...
                            $now = time();
                            $exam_time = $exam['starting_date'];
                            $time_diff = $exam_time - $now;
                            ?>
                            <div class="exam-timer" id="exam-timer-<?php echo $exam['id']; ?>" data-exam-date="<?php echo $exam['starting_date']; ?>" data-already-passed="<?php echo $already_passed ? '1' : '0'; ?>">
                                <?php
...
function updateExamsList(exams) {
    ...
    if (exams && Array.isArray(exams) && exams.length > 0) {
        ...
        $.each(exams, function(index, exam) {
            ...
            var examTime = exam.starting_date || exam.starting_timestamp || 0;
