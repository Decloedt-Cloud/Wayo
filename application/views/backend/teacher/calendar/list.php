...
generateTimeOptions(selectElement, selectedDate) {
    ...
    $.ajax({
        url: '<?php echo site_url('teacher/get_events'); ?>',
        type: 'GET',
        data: {
    ...
    loadClassesWithEvents(start, end) {
        $.ajax({
            url: '<?php echo site_url('teacher/get_user_school'); ?>',
            type: 'GET',
            ...
            success: (response) => {
                try {
                    ...
                    if (data.status === 'success') {
                        $.ajax({
                            url: '<?php echo site_url('teacher/get_classes_with_events'); ?>',
                            type: 'POST',
                            data: {
    ...
    if (!event) {
        ...
        $.ajax({
            url: '<?php echo site_url('teacher/get_event'); ?>',
            type: 'GET',
            data: {
                id: eventId,
                [csrfName]: csrfHash
    ...
    // S'abonner aux mises à jour WebSocket uniquement si la réunion n'est pas terminée
    if (isVisio && occurrenceData.meeting_id && !isExpired && this.ws.readyState === WebSocket.OPEN) {
        const subscribeMessage = JSON.stringify({
            action: 'subscribe',
            meetingID: occurrenceData.meeting_id
        });
        this.ws.send(subscribeMessage);
    } else if (isVisio && occurrenceData.meeting_id && isExpired) {
    } else if (isVisio && occurrenceData.meeting_id) {
        console.warn('WebSocket not connected, cannot subscribe to meetingID:', occurrenceData.meeting_id);
        this.showNotification('warning', 'Unable to connect to real-time updates. Using polling instead.');
    }

    // Démarrer le polling si la réunion est active, a un meeting_id et n'est pas terminée
    if (isVisio && occurrenceData.meeting_id && !isExpired) {
        this.pollParticipantCount(eventId, occurrenceData.meeting_id, occurrenceDate);
    } 
    // Se désabonner lorsque le modal est fermé
    $('#eventEditModal').off('hidden.bs.modal').on('hidden.bs.modal', () => {
        if (isVisio && occurrenceData.meeting_id && this.ws.readyState === WebSocket.OPEN) {
            const unsubscribeMessage = JSON.stringify({
                action: 'unsubscribe',
                meetingID: occurrenceData.meeting_id
            });
    ...
    if (event.school_id) {
        $.ajax({
            url: '<?php echo site_url('teacher/get_classes_by_school'); ?>',
            type: 'POST',
    ...
    $.ajax({
        url: '<?php echo site_url('teacher/create_event'); ?>',
        type: 'POST',
        data: formData,
        success: (response) => {
            try {
...
$('#eventForm').on('submit', (e) => {
    ...
    $.ajax({
        url: '<?php echo site_url('teacher/update_event'); ?>',
        type: 'POST',
        data: formData,
        success: (response) => {
            try {
        ...
        $('#deleteevent').on('click', () => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '<?php echo site_url('teacher/delete_event'); ?>',
                        type: 'POST',
...
updateParticipantUI(eventId, participantCount, isRunning) {
   ...
   startMeeting() {
    ...
    if (buttonText === '<?php echo get_phrase('Start Meeting'); ?>') {
        $.ajax({
            url: '<?php echo site_url('teacher/start_meeting'); ?>',
            type: 'POST',
            data: {
                event_id: eventId,
