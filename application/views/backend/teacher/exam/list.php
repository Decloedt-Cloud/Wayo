...
if (!empty($permitted_class_ids)) {
    ...
    // If teacher has no permitted classes, return 0
    $this->db->where('1', '0');
}
        ...
        <div class="filter-row">
            ...
            <div class="filter-group">
                ...
                <select name="class_id" id="class_id" class="form-control filter-select">
                    ...
                    <?php
                    // Get only permitted classes
                    if (!empty($permitted_class_ids)) {
                        $this->db->where_in('id', $permitted_class_ids);
                    } else {
                        // If no permitted classes, show none
                        $this->db->where('1', '0');
                    }
                    $this->db->where('school_id', $school_id);
            ...
            <?php foreach ($exams as $exam): ?>
                <div class="exam-item" data-exam-id="<?php echo $exam['id']; ?>">
                    ...
                    <div class="exam-actions">
                        <button type="button" class="btn-icon btn-edit-exam" data-exam-id="<?php echo $exam['id']; ?>">
...
function loadExamsPage(page) {
    ...
    $.ajax({
        url: '<?php echo site_url('teacher/get_exams_paginated'); ?>',
        type: 'GET',
        data: {
            page: page,
            class_id: classId,
...
function updateExamsList(exams) {
    ...
    // Réinitialiser les gestionnaires d'événements après la mise à jour de la liste
    initExamItemHandlers();
}
...
$(document).ready(function() {
    // Initialize exam item handlers on page load
    initExamItemHandlers();
