<?php
$school_id = school_id();
$schools = $this->db->get_where('schools', array('Etat' => 1, 'status' => 1, 'id !=' => 1))->result_array();
...
<script>
let currentPageSchool = 1;
const rowsPerPageSchool = 10;
...
function quickSearchSchool() {
    ...
    const totalPages = Math.ceil(totalRows / rowsPerPageSchool);
    ...
    // Calculate slice
    const startIndex = (currentPageSchool - 1) * rowsPerPageSchool;
    const endIndex = Math.min(startIndex + rowsPerPageSchool, totalRows);
...
// Detect typing to reset page to 1
document.getElementById('quickSearchSchool').addEventListener('input', function() {
    currentPageSchool = 1;
    quickSearchSchool();
});
