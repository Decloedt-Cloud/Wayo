<?php
$school_id = school_id();
$total_schools = $schools ? $schools->num_rows() : 0;
?>
...
<?php foreach($schools->result_array() as $school):
    $user = $this->db->get_where('users', array('school_id' => $school['id']))->row_array();
    $description = $school['description'];
    $category = $school['category'];
    $phone = $school['phone'];
?>
<div class="adm-list-item" data-name="<?php echo strtolower($school['name']); ?>" data-email="<?php echo strtolower($user['email']); ?>">
    ...
    <div class="adm-col-email">
        <div class="adm-email-display">
            ...
            <span title="<?php echo htmlspecialchars($user['email']); ?>">
                <?php echo strlen($user['email']) > 25 ? substr($user['email'], 0, 25) . '...' : $user['email']; ?>
            </span>
        </div>
    </div>
