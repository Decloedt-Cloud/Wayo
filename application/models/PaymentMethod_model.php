...
/**
 * PaymentMethod_model
 * Gestion des moyens de paiement par entité de facturation
 * Utilise payment_settings existant + billing_entity_payment_methods
 */
class PaymentMethod_model extends CI_Model
{
    ...
    public function get_for_entity($entity_id, $active_only = true)
    {
        ...
        $links = $this->db->get(self::LINK_TABLE)->result_array();
    ...
    public function link_to_entity($entity_id, $method_code, $options = [])
    {
        ...
        $existing = $this->db->get_where(self::LINK_TABLE, [
        ...
        if ($existing) {
            ...
            return $this->db->update(self::LINK_TABLE, $update_data);
        }

        // Créer nouveau lien
        return $this->db->insert(self::LINK_TABLE, [
            'billing_entity_id' => $entity_id,
            'method_code' => $method_code,
            'is_default' => isset($options['is_default']) ? (int)$options['is_default'] : 0,
            'is_active' => isset($options['is_active']) ? (int)$options['is_active'] : 1,
    ...
    {
        $this->db->where('billing_entity_id', $entity_id);
        $this->db->where('method_code', $method_code);
        return $this->db->delete(self::LINK_TABLE);
    }
    ...
    public function update_entity_methods($entity_id, $method_codes)
    {
        ...
        $this->db->where('billing_entity_id', $entity_id);
        $this->db->delete(self::LINK_TABLE);
    ...
    public function get_available_for_school($tax_residence, $school_id)
    {
        ...
        $school_settings = $this->db->get_where('payment_settings', ['school_id' => $school_id])->result_array();
        $configured = [];
