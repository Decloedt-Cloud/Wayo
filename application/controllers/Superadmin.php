...
class Superadmin extends CI_Controller
{
  ...
  public function permission($param1 = '', $param2 = '', $param3 = '')
  {
    ...
    if ($param1 == 'modify_permission') {
      ...
      $response_html = $this->load->view('backend/superadmin/permission/list', $page_data, TRUE);
  ...
  public function attendance($param1 = '', $param2 = '', $param3 = '')
  {
    ...
    if ($param1 == 'filter') {
      ...
      // $this->load->view('backend/superadmin/attendance/list', $page_data);
        // Charger la vue mise à jour
        $response_html = $this->load->view('backend/superadmin/attendance/list', $page_data, TRUE);
        // Préparer le nouveau jeton CSRF
        $csrf = array(
         'csrfName' => $this->security->get_csrf_token_name(),
         'csrfHash' => $this->security->get_csrf_hash(),
   ...
   public function quiz_result($param1 = '', $param2 = '')
   {
     ...
     if ($param1 == 'list') {
      ...
      //  $this->load->view('backend/superadmin/quiz/list', $page_data);
            // Charger la vue et capturer le contenu
            $html_content = $this->load->view('backend/superadmin/quiz/list', $page_data, TRUE);
  ...
  function promotion($param1 = "", $promotion_data = "")
  {
    ...
    if ($param1 == 'list') {
      ...
      // $this->load->view('backend/superadmin/promotion/list', $page_data);
        ...
        // Charger la vue et capturer le contenu
        $html_content = $this->load->view('backend/superadmin/promotion/list', $page_data, TRUE);
  ...
  public function website_update($param1 = "")
{
 ...
 if ($param1 == 'active') {
    ...
    if (!empty($referer)) {
        ...
        redirect(site_url('superadmin/dashboard'), 'location');
    }
}
  ...
  public function exam_results($exam_id = "", $student_id = "") {
    try {
        ...
        if (!$exam) {
            redirect(site_url('superadmin/exam'), 'refresh');
        }

        // Vérifier l'existence de l'étudiant et son association avec l'école
        $student_data = $this->db->get_where('students', ['id' => $student_id, 'school_id' => $exam['school_id']])->row_array();
        if (!$student_data) {
            redirect(site_url('superadmin/exam'), 'refresh');
        }
