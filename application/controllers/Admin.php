...
public function start_meeting()
{
    ...
    if ((string)$xml->returncode === "SUCCESS") {
        ...
        // Vérifier les enregistrements pour ce meeting
        ...
        if (!$curl_error) {
            ...
            if ($recordings_xml && (string)$recordings_xml->returncode === "SUCCESS") {
                foreach ($recordings_xml->recordings->recording as $recording) {
                    if ((string)$recording->state !== 'published') {
                        continue; // Ne stocker que les enregistrements publiés
                    }
                    $recording_id = (string)$recording->recordID;
                    $recording_start_time = (string)$recording->startTime; // Timestamp en millisecondes
                    $recording_end_time = (string)$recording->endTime; // Timestamp en millisecondes
                    $recording_url = (string)$recording->playback->format->url; // URL de l'enregistrement
                    $duration = (int)(($recording_end_time - $recording_start_time) / 60000); // Durée en minutes
