...
class Admin extends BaseController
{
    ...
    protected $services = [
        ...
        'moroccoB2BService' => 'moroccoB2BService',
    ...
    /**
     * @var \App\Libraries\MoroccoB2BService
     */
    protected $moroccoB2BService;
     ...
     * Trial expired flag
     */
    protected $trial_expired = false;

    /**
     * School data
     */
    protected $school_data = null;
     ...
     */
    protected function initializeSchoolAccess(): void
    {
        ...
        // Protection: unapproved community
        ...
        if (!$subscription_status['is_access_allowed']) {
            ...
            if (!in_array($current_method, $allowed_methods_trial)) {
                log_message('debug', "Admin - Access blocked for method '$current_method' (subscription: {$subscription_status['status']})");
                return redirect()->to(site_url('admin/dashboard'));
            }
        }

        // Auto-generate renewal invoice if needed
        ...
        if (!$subscription_status['is_access_allowed'] &&
            ...
            !in_array($current_method, $excluded_payment_methods)) {
				...
				$recent_invoice = $this->admin_model->get_by_query("
	...
	/*
	 * ALL PRIVATE METHODS COMMENTED OUT FOR TESTING
	 */
	...
	*/
	...
	private function get_system_base_currency() {
		return get_system_base_currency();
	}
	...
	private function get_stripe_currency($school_id = 1) {
		return get_stripe_currency($school_id);
	}
	...
	private function get_conversion_rate($from, $to) {
		...
		try {
			...
			// TODO: Replace with $this->fxrates_service = Config\Services::FxRatesService();
	...
	private function convertir_mad_vers_stripe($montant_mad, $devise_stripe) {
		...
		$latest_rates = $this->admin_model->get_by_query("SELECT * FROM fx_rates_daily ORDER BY rate_date DESC LIMIT 1");
		$latest_rates = !empty($latest_rates) ? $latest_rates[0] : null;
		if ($latest_rates) {
	...
	public function index()
	{
	   ...
	   public function get_appointments() {
		 ...
		 public function add_appointment() {
			...
			$sections = $this->request->getPost('sections'); // Tableau de sections sélectionnées
			...
			try {
				$this->admin_model->insert('appointments', $data);
	   ...
	   public function delete_appointment() {
		   ...
		   $this->admin_model->update('appointments', ['id' => $id], $appointments);
	   ...
	   //END TEACHER Create_Join bigbleubutton 
		...
		public function recording($param1 = '', $param2 = '', $param3 = '') 
		{
			...
			if (!$last_sync || ($current_time - $last_sync) > $sync_interval) {
				...
				$sql = "SELECT * FROM sessions_meetings WHERE school_id = ? AND created_at >= ? ORDER BY created_at DESC LIMIT 15";
				$meetings = $this->admin_model->get_by_query($sql, [$school_id, date('Y-m-d H:i:s', strtotime('-7 days'))]);
					...
					if ($recordings_xml && (string)$recordings_xml->returncode === "SUCCESS") {
						foreach ($recordings_xml->recordings->recording as $recording) {
							...
							if (!$existing) {
								...
								$this->admin_model->insert('recordings', $recording_data);
							} else {
								$this->admin_model->update('recordings', ['recording_id' => $recording_id], [
									'recording_url' => $recording_url,
									'updated_at' => date('Y-m-d H:i:s')
								]);
							}
			...
			$sql = "SELECT r.*, c.name as class_name FROM recordings r LEFT JOIN classes c ON r.class_id = c.id";
			if (!empty($conditions)) {
				$sql .= " WHERE " . implode(' AND ', $conditions);
			}
			$sql .= " ORDER BY r.created_at DESC";
			$recordings = $this->admin_model->get_by_query($sql, $params);
			...
			public function delete_recording()
{
    ...
    $existing = $this->admin_model->get_row('recordings', ['recording_id' => $recording_id]);
    ...
    $delete_result = $this->admin_model->delete('recordings', ['recording_id' => $recording_id]);
    ...
    if ($db_error['code'] == 0 && $delete_result['affected_rows'] > 0) {
        ...
        log_message('error', 'delete_recording - Failed to delete recording: recording_id=' . $recording_id . ', DB Error: ' . json_encode($db_error));
        $response = [
            'status' => 'error',
    ...
    $this->output->set_content_type('application/json')->set_output(json_encode($response));
		   ...
		   public function delete_appointment_and_recording($appointment_id)
		   {
			   ...
			   if (empty($appointment_id)) {
				   ...
				   return redirect()->to(site_url('admin/Recording'));
				   return;
			   }
	...
	public function dashboard()
	{
		...
		// $this->msg91_model->clickatell();
			...
			if ($school_id) {
				...
				$recent_paid_invoice = $this->admin_model->get_by_query("
					SELECT id, paid_at 
					FROM invoices 
					WHERE school_id = ? 
					AND payment_type = 'subscription_admin' 
					AND status = 'paid'
					AND paid_at > UNIX_TIMESTAMP() - 300
    ...
    public function subscription_admin($param1 = "", $school_id = "")
    {
      ...
      if ($param1 == 'assigned') {
       ...
       // Stocker les données de l'inscription dans la session pour un accès ultérieur
          ...
          $this->loadModel('Invoice_model', 'invoice_model');
          $invoice_result = $this->invoice_model->ensureSubscriptionInvoice($school_id);
        ...
        return redirect()->to(site_url('admin/payment/' . $invoice_id));
	...
	public function payment($param1 = "",$invoice_id = ""){
        ...
        $school = $this->admin_model->get_school_by_id($page_data['invoice_details']['school_id']);
        ...
        if ($page_data['invoice_details']['status'] == 'paid') {
            ...
            return redirect()->to('/Student/invoice');
        }
        ...
        $page_data['user_details'] = $this->db_helper_model->get_by_id('users', $student_id);
        ...
        $invoice = $this->db_helper_model->get('invoices', ['id' => $this->uri->segment(4)], 'row');
	...
	public function payment_success($payment_method = "", $invoice_id = "", $amount_paid = "", $reference = "") {
		...
		$school = $this->admin_model->get_school_by_id($invoice_details['school_id']);
		...
		if ($invoice_details['payment_type'] === 'subscription_admin') {
			...
			$client_sent_currency = $this->request->getPost('currency') ?? $payment_currency;
		...
		if ($payment_status === true) {
			...
			$invoice_current = $this->admin_model->get_row('invoices', ['id' => $invoice_id]);
			...
			if ($due_amount <= $amount_for_comparison) {
				...
				if (isset($invoice_details['payment_type']) && $invoice_details['payment_type'] === 'subscription_admin') {
					...
					$this->admin_model->update('invoices', ['id' => $invoice_id], $updater);
					...
					if ($is_community_payment) {
						...
						$existing_admin = $this->db_helper_model->get('admins', [
							'user_id' => $user_id,
						...
						if (!$existing_admin) {
							...
							$this->admin_model->insert('admins', [
								'user_id' => $user_id,
								'school_id' => $community_school_id,
								'created_at' => time()
							]);
	...
	public function filter_recordings()
	{
		...
		$conditions = [];
		$params = [];
		$conditions[] = 'appointments.Etat = ?';
		$params[] = 1;
		$conditions[] = 'appointments.school_id = ?';
		$params[] = $schoolID;
	
		if (!empty($meeting_name)) {
			$conditions[] = 'appointments.title LIKE ?';
			$params[] = '%' . $meeting_name . '%';
		}
		...
		if (!empty($date_range)) {
			...
			if (count($dates) === 2) {
				$conditions[] = 'appointments.start_date >= ?';
		...
		$appointments = $this->admin_model->get_by_query($sql, $params);
		...
		foreach ($appointments as $appointment) {
			...
			if (!empty($appointment['section'])) {
				...
				foreach ($section_ids as $id) {
					$name = $this->admin_model->get_section_name($id);
	...
	public function class_room($param1 = '', $param2 = '')
	{
		...
		if ($param1 == 'list') {
			return view('backend/admin/class_room/list');
		}
    ...
    $response = array(
      ...
      echo json_encode($response);
		...
		if ($param1 == 'delete') {
			$teacher_id = $this->admin_model->get_teacher_by_user_id($param2);
	...
	public function accountant($param1 = '', $param2 = '')
	{
		...
		if ($param1 == 'status') {
			...
			$status = $param3;
			$this->admin_model->update('users', ['id' => $accountant_id], ['status' => $status]);
	...
	public function attendance($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'filter') {
			...
			$page_data['class_id'] = $this->request->getPost('class_id');
		
			$page_data['month'] = $this->request->getPost('month');
			$page_data['year'] = $this->request->getPost('year');
	...
	public function student($param1 = '', $param2 = '', $param3 = '', $param4 = '', $param5 = '')
  {
   ...
   if ($param1 == 'edit') {
    ...
    if ($param1 == 'status') {
      $this->admin_model->update('users', ['id' => $param3], ['status' => $param4]);
      $response = array(
        'status' => true,
	...
	public function exam($param1 = '', $param2 = '')
{
    ...
    if ($param1 == 'list') {
        return view('backend/admin/exam/list');
    }
  ...
  public function smtp_settings($param1 = "", $param2 = "")
  {
	...
	public function mark($param1 = '', $param2 = '')
	{
		...
		if ($param1 == 'list') {
			...
			// Charger la vue mise à jour
			$response_html = view('backend/admin/mark/list', $page_data);
			// Préparer le nouveau jeton CSRF
			$csrf = array(
					'csrfName' => $this->security->get_csrf_token_name(),
					'csrfHash' => $this->security->get_csrf_hash(),
	...
	public function quiz_result($param1 = '', $param2 = '')
	{
	  ...
	  if ($param1 == 'list') {
		...
		// $this->load->view('backend/admin/quiz/list', $page_data);
			...
			// Charger la vue mise à jour
			$response_html = view('backend/admin/quiz/list', $page_data);
			// Préparer le nouveau jeton CSRF
			$csrf = array(
					'csrfName' => $this->security->get_csrf_token_name(),
					'csrfHash' => $this->security->get_csrf_hash(),
	...
	function promotion($param1 = "", $promotion_data = "")
	{
		...
		if ($param1 == 'list') {
			...
			$page_data['class_from_details'] = $this->crud_model->get_classes($this->request->getPost('class_id_from'))->row_array();
			$page_data['class_to_details'] = $this->crud_model->get_classes($this->request->getPost('class_id_to'))->row_array();
			$page_data['session_from_details'] = $this->crud_model->get_session($this->request->getPost('session_from'))->row_array();
			$page_data['session_to_details'] = $this->crud_model->get_session($this->request->getPost('session_to'))->row_array();
			$page_data['enrolments'] = $this->crud_model->get_student_list()->result_array();
			// $this->load->view('backend/admin/promotion/list', $page_data);
			// Charger la vue mise à jour
			$response_html = view('backend/admin/promotion/list', $page_data);
			// Préparer le nouveau jeton CSRF
			$csrf = array(
					'csrfName' => $this->security->get_csrf_token_name(),
					'csrfHash' => $this->security->get_csrf_hash(),
  ...
  public function export($param1 = "", $date_from = "", $date_to = "", $selected_class = "", $selected_status = "")
  {
    ...
    public function invoice_pdf($invoice_id = "")
    {
      ...
      ob_start();
      return view('backend/admin/invoice/invoice_pdf', $page_data);
      $html = ob_get_clean();
	...
	public function expense($param1 = "", $param2 = "")
	{
		...
		if ($param1 == 'export') {
			$type = htmlspecialchars($this->request->getGet('type'));
			$dateRange = $this->request->getGet('date');
			$expense_category_id = htmlspecialchars($this->request->getGet('expense_category_id'));
			...
			// Get school currency
			$school_currency = $this->admin_model->get_settings_school(school_id()) ?? 'EUR';
			...
			if ($type == 'csv') {
				...
				foreach ($expenses as $expense) {
					$category = $this->admin_model->get_expense_category($expense['expense_category_id']);
			...
			if ($type == 'pdf') {
				...
				// Render HTML
				ob_start();
				return view('backend/admin/expense/export_pdf', $page_data);
				$html = ob_get_clean();
    ...
    public function online_admission($param1 = "", $user_id = "")
{
    if ($param1 == 'assigned') {
        ...
        $user_id = $this->admin_model->get_student_user_id($data['student_id']);
        $this->email_model->approved_online_admission($data['student_id'], $user_id);

        if (!empty($school_id)) {
            $this->admin_model->update('students', ['id' => $data['student_id'], 'school_id' => $school_id], ['status' => 1]);
        } else {
            $this->admin_model->update('students', ['user_id' => $user_id], ['status' => 1]);
        }

        $this->session->set_flashdata('flash_message', get_phrase('admission_request_has_been_updated'));
        return redirect()->to(site_url('admin/online_admission'));
    }

    if ($param1 == 'delete') {
        $this->admin_model->delete('students', ['user_id' => $user_id]);
    ...
    $sql = "SELECT students.id as student_id, students.user_id, students.school_id, users.id, users.name, users.email 
            FROM students 
            JOIN users ON users.id = students.user_id 
            WHERE students.status = 0 
            AND students.school_id = ?";
    $page_data['applications'] = $this->admin_model->get_by_query($sql, [$this->session->userdata('school_id')]);
...
public function online_admission_school($param1 = "", $school_id = "")
{
    if ($param1 == 'approved') {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('admission_request_has_been_updated'));
        return redirect()->to(site_url('admin/online_admission_school'));
    }
    if ($param1 == 'delete') {
        $data['Etat'] = 0;
        $this->db->table('schools')->update($data, ['id' => $school_id]);
        $this->session->set_flashdata('flash_message', get_phrase('admission_data_deleted_successfully'));
        return redirect()->to(site_url('admin/online_admission_school'));
    }
    $page_data['schools'] = $this->admin_model->get_inactive_schools();
	...
	function driver($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'delete') {
			$driver_id = $this->admin_model->get_driver_by_user_id($param2);
			$response = $this->driver_model->delete_driver($param2, $driver_id);
			echo $response;
		}

		if ($param1 == 'list') {
			return view('backend/admin/driver/list');
		}
	...
	function vehicle($param1 = '', $param2 = '', $param3 = '')
	{
		...
		if ($param1 == 'list') {
			return view('backend/admin/vehicle/list');
		}
	...
	function assign_student($param1 = '', $param2 = '', $param3 = '', $param4 = '')
	{
		...
		if ($param1 == 'list') {
			return view('backend/admin/assign_student/list');
		}

		if ($param1 == 'filter') {
			$page_data['parent_category'] = $param2;
			$page_data['child_category'] = $param3;
			return view('backend/admin/assign_student/list', $page_data);
		}
	...
	{
		$sql = "SELECT enrols.*, users.name 
				FROM enrols 
				JOIN students ON enrols.student_id = students.id 
				JOIN users ON students.user_id = users.id 
				WHERE enrols.class_id = ? 
				AND enrols.school_id = ?";
		$total_students = $this->admin_model->get_by_query($sql, [ $class_id, school_id()]);
	...
	function getAdditionalCategory($category)
	{
		$name = 'name';
		if ($category == 'class') {
			$result = $this->admin_model->get_where_result('classes', ['school_id' => school_id()]);
		} elseif ($category == 'vehicle') {
			$name = 'vh_num';
			$result = $this->admin_model->get_where_result('vehicles', ['school_id' => school_id()]);
		} elseif ($category == 'driver') {
			$sql = "SELECT drivers.*, users.name FROM drivers JOIN users ON drivers.user_id = users.id WHERE drivers.school_id = ?";
			$result = $this->admin_model->get_by_query($sql, [school_id()]);
		}
	...
	public function exam_results($exam_id = "", $student_id = "") {
		try {
			...
			$school_id = $this->session->userdata('school_id');
			if (!$school_id) {
				return redirect()->to(site_url('admin/exam'));
			}
	
			// Récupérer les détails de l'examen
			$sql = "SELECT exams.*, classes.name as class_name, schools.name as school_name 
					FROM exams 
					LEFT JOIN classes ON exams.class_id = classes.id 
					LEFT JOIN schools ON exams.school_id = schools.id 
					WHERE exams.id = ? 
					AND exams.school_id = ?";
			$exam = $this->admin_model->get_by_query($sql, [$exam_id, $school_id]);
			$exam = !empty($exam) ? $exam[0] : null;
			...
			$student_data = $this->db_helper_model->get_by_id('students', $student_id);
			if (!$student_data) {
				return redirect()->to(site_url('admin/exam'));
			}
			...
			$sql = "SELECT er.*, eq.title as question_title 
					...
					WHERE er.exam_id = ? 
					AND er.user_id = ?";
			$submitted_answers = $this->admin_model->get_by_query($sql, [$exam_id, $student_data['user_id']]);
	...
	public function filter_exams()
{
    ...
    $conditions = [];
    $params = [];
    $conditions[] = 'exams.school_id = ?';
    $params[] = $school_id;
    $conditions[] = 'exams.session = ?';
    $params[] = $session;
    
    if (!empty($class_id)) {
        $conditions[] = 'exams.class_id = ?';
        $params[] = $class_id;
    }
    
    if (!empty($date_range)) {
        $conditions[] = 'exams.starting_date >= ?';
        $params[] = $date_from;
        $conditions[] = 'exams.starting_date <= ?';
        $params[] = $date_to;
    }
    
    $sql = "SELECT exams.*, classes.name as class_name FROM exams LEFT JOIN classes ON exams.class_id = classes.id";
    if (!empty($conditions)) {
        $sql .= " WHERE " . implode(' AND ', $conditions);
    }
    $sql .= " ORDER BY exams.starting_date DESC";
    $exams = $this->admin_model->get_by_query($sql, $params);
...
public function get_exams_paginated()
{
    ...
    if (!empty($date_range) && $date_from && $date_to) {
        $conditions[] = 'starting_date >= ?';
        $conditions[] = 'starting_date <= ?';
    }
    $total_exams = $this->admin_model->count_all('exams', $conditions, [$school_id, $session, $class_id ?? null, $date_from ?? null, $date_to ?? null]);
    ...
    $sql .= " ORDER BY exams.id DESC LIMIT ? OFFSET ?";
    $params[] = $limit;
    $params[] = $offset;
    $exams = $this->admin_model->get_by_query($sql, $params);
...
public function get_user_school() {
    ...
    $school = $this->admin_model->get_school_by_id($user['school_id']);
    if (!$school) {
...
public function create_event() {
    ...
    if (!$this->admin_model->school_exists($data['school_id'])) {
        echo json_encode([
            'status' => 'error',
	...
	foreach ($participants as $participant) {
        ...
        if ($participant['type'] === 'class') {
            if (!$this->admin_model->class_exists($participant['id'], $data['school_id'])) {
                echo json_encode([
                    'status' => 'error',
            ...
            if (!$this->admin_model->user_exists($participant['id'])) {
                echo json_encode([
                    'status' => 'error',
    ...
    try {
        $event_id = $this->admin_model->insert('event_calendars', $data);
		foreach ($participants as $participant) {
            $this->admin_model->insert('participants', [
                'event_id' => $event_id,
    ...
    public function update_event() {
        ...
        if (!$this->db_helper_model->get('event_calendars', ['id' => $event_id], 'row')) {
            $csrf = [
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash(),
            ];
    ...
    if ($end_date_obj < $start_date_obj) {
        ...
        try {
            $this->admin_model->update('event_calendars', ['id' => $event_id], $data);
            // Mettre à jour les rendez-vous associés
            $this->admin_model->update('appointments', ['event_id' => $event_id], [
                'title' => $data['title'],
                'start_date' => $data['starting_date'],
                'school_id' => $data['school_id'],
                'visio' => $data['visio']
            ]);
			 $this->admin_model->delete('participants', ['event_id' => $event_id]);
            foreach ($participants as $participant) {
                $this->admin_model->insert('participants', [
                    'event_id' => $event_id,
                    'guest' => $participant['id'],
    ...
    public function delete_event() {
        ...
        try {
            ...
            $appointments = $this->db_helper_model->get('appointments', ['event_id' => $event_id], 'result_array');
            foreach ($appointments as $appointment) {
                $existing_meeting = $this->admin_model->get_session_meeting_by_appointment($appointment['id']);
            ...
            $this->admin_model->delete('appointments', ['event_id' => $event_id]);

			$this->admin_model->delete('participants', ['event_id' => $event_id]);

            $this->admin_model->delete('event_calendars', ['id' => $event_id]);
    ...
    $conditions = [];
    $params = [];
    $conditions[] = 'event_calendars.school_id = ?';
    $params[] = $school_id;

    if ($event_id) {
        $conditions[] = 'event_calendars.id = ?';
        $params[] = $event_id;
    } else {
        $conditions[] = '(event_calendars.starting_date <= ? AND (event_calendars.ending_date >= ? OR event_calendars.ending_date IS NULL))';
        $params[] = $end_date;
        $params[] = $start_date;
    }

    if ($class_id) {
        $sql .= " LEFT JOIN participants ON event_calendars.id = participants.event_id";
        $conditions[] = 'participants.guest = ?';
        $params[] = $class_id;
        $conditions[] = 'participants.type = ?';
        $params[] = 'class';
    }
    ...
    $events = $this->admin_model->get_by_query($sql, $params);
    ...
    foreach ($events as $event) {
        ...
        $event['participants'] = [];
        $participants = $this->admin_model->get_participants_by_event($event['id']);
        foreach ($participants as $participant) {
            $participant_data = [
            ...
            if ($participant['type'] === 'class') {
                $class = $this->admin_model->get_class_by_id($participant['guest']);
                $participant_data['name'] = $class ? $class->name : 'Unknown';
            } elseif ($participant['type'] === 'individual') {
                $user = $this->admin_model->get_user_by_id($participant['guest']);
        ...
        if ($event['visio'] == 1) {
            $sql = "SELECT start_date, meeting_id FROM appointments WHERE event_id = ? AND etat = 1 AND DATE(start_date) >= ? AND DATE(start_date) <= ?";
            $appointments = $this->admin_model->get_by_query($sql, [$event['id'], $start_date, $end_date]);
...
public function start_meeting(){
    ...
    $this->admin_model->update('appointments', ['event_id' => $event_id, 'DATE(start_date) !=' => $occurrence_date, 'Etat' => 1], ['Etat' => 0]);

    // Vérifier si un appointment actif existe pour cette occurrence
    $appointment = $this->admin_model->get_row('appointments', [
        'event_id' => $event_id,
    ...
    if ($appointment && $appointment['meeting_id']) {
        $meeting = $this->admin_model->get_session_meeting($appointment['meeting_id'], $appointment_id);
        if ($meeting) {
            ...
            if ($is_running_xml && (string)$is_running_xml->returncode === "SUCCESS" && (string)$is_running_xml->running === "true") {
                ...
                $participants = $this->admin_model->get_where_result('appointment_participants', ['appointment_id' => $appointment_id]);
                ...
                $this->admin_model->update('appointments', ['id' => $appointment_id], ['Etat' => 0]);
            }
        }
    }

    // Retrieve all participants (classes and users) for the event
    $participants = $this->admin_model->get_where_result('participants', ['event_id' => $event_id]);
    $appointment_data = [
        'event_id' => $event_id,
        'title' => $event['title'],
    ...
    $appointment_id = $this->admin_model->insert('appointments', $appointment_data);

	foreach ($participants as $participant) {
        $this->admin_model->insert('appointment_participants', [
            'appointment_id' => $appointment_id,
    ...
    if ((string)$xml->returncode === "SUCCESS") {
        ...
        $this->admin_model->insert('sessions_meetings', $meeting_data);

        // Mettre à jour l'appointment avec le nouveau meeting_id
        $this->admin_model->update('appointments', ['id' => $appointment_id], ['meeting_id' => $new_meeting_id]);
...
public function get_classes_by_school()
{
    ...
    $classes = $this->admin_model->get_classes_by_school($school_id);
    if (empty($classes)) {
...
public function get_school_data() {
    ...
    try {
        ...
        if (!empty($class_ids)) {
            $conditions[] = 'e.class_id NOT IN (' . implode(',', array_fill(0, count($class_ids), '?')) . ')';
        ...
        $students = $this->admin_model->get_by_query($sql, $params);
        $users = array_merge($users, $students);

        // 2. Récupérer les enseignants
        $sql_teachers = "SELECT DISTINCT(u.id), u.name, 'teacher' as type, u.role 
                         FROM users u 
                         ...
                         AND u.id != ? 
                         ORDER BY u.name ASC";
        $teachers = $this->admin_model->get_by_query($sql_teachers, [$school_id, $current_user_id]);
        $users = array_merge($users, $teachers);

        // 3. Récupérer les admins et superadmins
        $sql_admins = "SELECT DISTINCT(u.id), u.name, u.role as type, u.role 
                       FROM users u 
                       WHERE u.school_id = ? 
                       AND u.status = 1 
                       AND u.role IN ('admin', 'superadmin') 
                       ORDER BY u.name ASC";
        $admins = $this->admin_model->get_by_query($sql_admins, [$school_id]);
    ...
    public function check_teacher_email()
    {
        ...
        $existing_teacher = $this->admin_model->get_row('user_schools', [
            'user_id' => $user['id'],
            'school_id' => $school_id,
            'role' => 'teacher'
        ]);
