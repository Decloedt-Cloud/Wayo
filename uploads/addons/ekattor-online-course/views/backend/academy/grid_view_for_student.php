...
<div class="modern-dashboard">
            ...
            <div class="modern-card filter-card">
                <div class="modern-card-body">
                    <form class="row align-items-end modern-filter-form" action="javascript:void(0)">
                        ...
                        <input type="hidden" name="<?=csrf_token();?>" value="<?=csrf_hash();?>" id="csrf_token_filter" />
                        <?php
                        $as_result_array = static function ($queryResult): array {
                            ...
                            if (is_object($queryResult)) {
                                if (method_exists($queryResult, 'result_array')) {
                                    $rows = $queryResult;
                                    return is_array($rows) ? $rows : [];
                                }
                                if (method_exists($queryResult, 'getResultArray')) {
                                    $rows = $queryResult->getResultArray();
                                    return is_array($rows) ? $rows : [];
                                }
                                if (method_exists($queryResult, 'getResult')) {
                        ...
                        <div class="col-md-4 col-lg-3 mb-3 mb-md-0">
                            ...
                            <select class="form-control modern-select" name="class_id" id="class_id_course">
                                ...
                                if ($selected_school_id != "all") {
                                    $classes_query = db()->table('classes')->select('id, name')->where('school_id', school_id())->get();
                                    $classes_list = $as_result_array($classes_query);
                        ...
                        <?php if (session()->get('teacher_login') != 1): ?>
                            <div class="col-md-4 col-lg-3 mb-3 mb-md-0">
                                ...
                                <?php
                                $teacher_ids = [];
                                $coursesBuilder = $this->db->table('course')
                                    ->select('course.id')
                                    ->join('course_classes', 'course_classes.course_id = course.id')
                                    ->where('course.school_id !=', null);

                                if ($selected_school_id != 'all')
                                    $coursesBuilder->where('course.school_id', $selected_school_id);
                                if ($selected_class_id != 'all')
                                    $coursesBuilder->where('course_classes.class_id', $selected_class_id);

                                $courses = $as_result_array($coursesBuilder->get());

                                $lms_model = $this->lms_model ?? model('App\Models\addons\Lms_model');
                                foreach ($courses as $course) {
                                    $course_teachers = [];
                                    if (is_object($lms_model) && method_exists($lms_model, 'get_teachers_by_course')) {
                                        $course_teachers = $lms_model->get_teachers_by_course($course['id']);
                                        if (is_object($course_teachers) && method_exists($course_teachers, 'result_array')) {
                                            $course_teachers = $course_teachers;
                                        } elseif (is_object($course_teachers) && method_exists($course_teachers, 'getResultArray')) {
                                            $course_teachers = $course_teachers->getResultArray();
                                        }
                                    }
                                    if (!is_array($course_teachers)) {
                                        $course_teachers = [];
                                    }
                                    ...
                                    foreach ($course_teachers as $teacher) {
                                        ...
                                        if (isset($teacher['id'], $teacher['name'])) {
    ...
    $studentIdsBuilder = $this->db->table('students')
        ->select('id')
        ->where(['user_id' => $user_id, 'status' => 1]);
    if ($selected_school_id != 'all') {
        $studentIdsBuilder->where('school_id', $selected_school_id);
    }
    $student_ids = $as_result_array($studentIdsBuilder->get());
    ...
    if (!empty($student_ids)) {
        ...
        $classesBuilder = $this->db->table('classes')
            ->select('classes.*, students.user_id AS student_user_id, students.id AS real_student_id, students.school_id AS student_school_id')
            ...
            ->orWhere('classes.date_fin IS NULL', null, false)
            ->orWhere('classes.date_fin', '0000-00-00')
            ->groupEnd();

        if ($selected_school_id != 'all')
            $classesBuilder->where('classes.school_id', $selected_school_id);
        if ($selected_class_id != 'all')
            $classesBuilder->where('classes.id', $selected_class_id);

        $classes = $as_result_array($classesBuilder->get());
        // Ensure each class appears once even if joins return duplicate rows.
        if (!empty($classes)) {
            $unique_classes = [];
            foreach ($classes as $class_row) {
                $class_id_key = $class_row['id'] ?? null;
                if ($class_id_key !== null && !isset($unique_classes[$class_id_key])) {
                    $unique_classes[$class_id_key] = $class_row;
                }
            }
            $classes = array_values($unique_classes);
        }
    }
    ?>
    ...
    <?php if (!empty($classes)): ?>
        ...
        $student_profile_ids = array_values(array_unique(array_filter(array_map(static function ($class_row) {
            return $class_row['real_student_id'] ?? null;
        }, $classes))));

        // 2. Batch Fetch Enrollment Counts (Global for class)
        $enrollment_counts = [];
        if (!empty($class_ids)) {
            $query_counts = $as_result_array(
                $this->db->table('enrols')
            ...
            foreach ($query_counts as $row) {
                ...
                $enrollment_counts[$row['class_id']] = (int) ($row['count'] ?? $row['COUNT(*)'] ?? 0);
            }
        }
        ...
        $vat_settings_map = [];
        if (!empty($school_ids)) {
            $query_settings = $as_result_array(
                $this->db->table('settings_school')
                    ->select('school_id, system_currency, vat_enabled, vat_rate')
        ...
        if (!empty($class_ids) && !empty($student_profile_ids)) {
            $query_enrols = $as_result_array(
                $this->db->table('enrols')
                    ->select('class_id, student_id')
                    ->whereIn('class_id', $class_ids)
                    ->whereIn('student_id', $student_profile_ids)
                    ->get()
            );
        ...
        <div class="row fade-up delay-2">
            <?php foreach ($classes as $class):
                ...
                $student_profile_id = $class['real_student_id'] ?? 0; // ID from students table for invoices/enrollments
                $student_user_id = $class['student_user_id'] ?? 0; // user_id for other purposes
                ...
                <div class="col-md-6 col-lg-4 col-xl-3 mb-4">
                    <div class="modern-card course-card">
                        ...
                        <div class="course-img-wrapper">
                            <?php $class_photo = $class['photo'] ?? 'placeholder.png'; ?>
                            <img src="<?= base_url('uploads/class/' . ($class_photo ?: 'placeholder.png')); ?>" 
                                 class="course-img" 
                                 alt="<?= $class['name'] ?? 'class'; ?>"
...
function filterStudentClasses() {
    ...
    var user_id = userSelect.length ? userSelect.val() : 'all';
    var status = '<?= esc($selected_status ?? 'all') ?>';
    var school_id = '<?= esc($selected_school_id ?? 'all') ?>';
    var url = '<?= site_url('addons/courses') ?>'
        + '?class_id=' + encodeURIComponent(class_id || 'all')
        + '&user_id=' + encodeURIComponent(user_id || 'all')
        + '&status=' + encodeURIComponent(status || 'all')
        + '&school_id=' + encodeURIComponent(school_id || 'all')
        + '&only_list=true';
