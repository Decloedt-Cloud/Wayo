...
class Courses extends BaseController {
  ...
  // Keep duplicate-click lock short; long-running protection is handled by request lifecycle.
  const AI_GENERATION_LOCK_DURATION = 30; // 30 seconds
  ...
  public function __construct(){
    ...
    $this->deepseekApiKey = (string) (function_exists('env') ? env('ai.deepseekApiKey', '') : '');
    ...
    if($teacher_login == 1 || $superadmin_login == 1 || $admin_login == 1 || $student_login == 1){
        ...
        if ($student_login == 1) {
            ...
            $student_check = $this->lms_model->get_student_by_user_and_school($user_id, $current_school_id);
            
            if (!$student_check || $student_check['status'] != 1) {
                //session()->setFlashdata('error_message', get_phrase('you_must_wait_for_the_community_owner_approval'));
  ...
  private function initHtmlPurifier() {
    ...
    $config->set('HTML.Allowed', 
      ...
      'iframe[src|width|height|frameborder|class],'.
      'hr'
    );
  ...
  public function index($param1 = '', $param2 = ''){
    ...
    if($param1 == 'create'){
      ...
      session()->setFlashdata('flash_message', get_phrase('course_added_successfully'));
      return redirect()->to(site_url('addons/courses/course_edit/' . $course_id));
    }

    if($param1 == 'update'){
      $this->student_access_denied();
      $this->lms_model->course_edit($param2);
      session()->setFlashdata('flash_message', get_phrase('course_updated_successfully'));
      return redirect()->to(site_url('addons/courses'));
    }
    ...
    if(empty($param1)){
      $normalize_filter = static function ($value, $default = 'all') {
        if ($value === null) {
          return $default;
        }
      ...
      $page_data['selected_class_id']  = $normalize_filter($this->request->getGet('class_id'));
      $page_data['selected_user_id']   = $normalize_filter($this->request->getGet('user_id'));
      $page_data['selected_status']    = $normalize_filter($this->request->getGet('status'));
      $page_data['selected_school_id'] = $normalize_filter($this->request->getGet('school_id'));
      $only_list = strtolower($normalize_filter($this->request->getGet('only_list'), 'false'));
      ...
      if($only_list === 'true'){
        if(session()->get('student_login') == 1){ 
          ...
          return view('backend/academy/list', $page_data);
        }
      }else{
  ...
  public function course_add(){
    ...
    $page_data['all_teachers']    = $this->user_model->get_all_teachers();
    $page_data['classes']     = $this->crud_model->get_classes();
    $page_data['categories']  = $this->lms_model->get_all_categories();
    
    // Récupérer la catégorie de l'école actuelle
    $current_school_id = school_id();
    $school_category = '';
    if ($current_school_id) {
        $school = $this->lms_model->get_school_by_id($current_school_id);
  ...
  public function course_edit($course_id = ""){
    ...
    // $page_data['subjects']        = $this->db->get_where('subjects', array('class_id' => $page_data['course']['class_id']));
    // Trouver la première leçon qui n'est pas un quiz
    $first_non_quiz_lesson = $this->lms_model->get_first_non_quiz_lesson($course_id);
    
    // Si aucune leçon non-quiz trouvée, prendre la première leçon disponible
    if (empty($first_non_quiz_lesson)) {
        $first_non_quiz_lesson = $this->lms_model->get_lesson_by_course($course_id);
    }
    ...
    //     $page_data['subjects'] = $this->db->get_where('subjects', array('class_id' => $first_class_id));
    // } else {
    //     $page_data['subjects'] = [];
    // }
    $page_data['folder_name']     = 'academy';
  ...
  public function filter() {
      ...
      $school_id = $this->request->getGet('school_id'); // Optional, defaulting to empty/all if not passed
  ...
  public function course_sections($param1 = "", $param2 = "", $param3 = "") {
    ...
    if ($param2 == 'add') {
      ...
      session()->setFlashdata('flash_message', get_phrase('section_has_been_added_successfully'));
    }
    elseif ($param2 == 'edit') {
      $this->lms_model->edit_course_section($param3);
      session()->setFlashdata('flash_message', get_phrase('section_has_been_updated_successfully'));
    }
    ...
    return redirect()->to(site_url('addons/courses/course_edit/'.$param1));
  }
  ...
  public function ajax_update_section($section_id) {
    ...
    $section = $this->lms_model->get_course_section_by_id($section_id);
    if (!$section) {
  ...
  public function quizes($course_id = "", $action = "", $quiz_id = "") {
    ...
    if ($action == 'add') {
      ...
      session()->setFlashdata('flash_message', get_phrase('quiz_has_been_added_successfully'));
    }
    elseif ($action == 'edit') {
      $this->lms_model->edit_quiz($quiz_id);
      session()->setFlashdata('flash_message', get_phrase('quiz_has_been_updated_successfully'));
    }
  ...
  public function lessons($course_id = "", $param1 = "", $param2 = "") {
    ...
    if ($param1 == 'add') {
      ...
      session()->setFlashdata('flash_message', get_phrase('lesson_has_been_added_successfully'));
      return redirect()->to('addons/courses/course_edit/'.$course_id);
    }
    elseif ($param1 == 'edit') {
      $this->student_access_denied();
      $this->lms_model->edit_lesson($param2);
      session()->setFlashdata('flash_message', get_phrase('lesson_has_been_updated_successfully'));
      return redirect()->to('addons/courses/course_edit/'.$course_id);
    }
  ...
  public function get_lesson_content($lesson_id = "") {
    ...
    $lesson = $this->lms_model->get_lesson_by_id($lesson_id);
  ...
  public function get_quiz_content($quiz_id) {
    ...
    $quiz = $this->lms_model->get_quiz_by_id($quiz_id);
    
    if ($quiz) {
      // Get questions for this quiz
      $questions = $this->lms_model->get_questions_by_quiz($quiz_id, true);
  ...
  public function save_lesson($lesson_id = "") {
    ...
    if (empty($lesson_id)) {
      ...
      $new_lesson_id = $this->lms_model->insert_lesson($data);
      ...
      $result = $this->lms_model->update_lesson($lesson_id, $data);
  ...
  public function save_quiz($quiz_id = "") {
    ...
    $section_id = $this->request->getPost('section_id');
    $course_id = $this->request->getPost('course_id');
    $questions_json = $this->request->getPost('questions');
    ...
    if (empty($quiz_id)) {
      ...
      $target_quiz_id = $this->lms_model->insert_lesson($data);
      ...
      $this->lms_model->update_lesson($quiz_id, $data);
    }
    ...
    if (!empty($questions_json)) {
      ...
      if (is_array($questions) && count($questions) > 0) {
        ...
        if (!empty($quiz_id)) {
          $this->lms_model->delete_questions_by_quiz($quiz_id);
        }
        ...
        foreach ($questions as $q) {
          ...
          $this->lms_model->insert_question($question_data);
        }
      }
    }
  ...
  public function ajax_sort_section() {
    ...
    $start_order = (int) esc($this->request->getPost('startOrder')) ?? 1;
  ...
  public function ajax_move_section() {
    ...
    $target_position = (int) $this->request->getPost('target_position');
  ...
  public function ajax_get_section($course_id){
    $page_data['course_sections'] = $this->toArrayResult($this->lms_model->get_section('course', $course_id));
    return view('backend/academy/ajax_get_section', $page_data);
  }
...
public function manage_multiple_choices_options() {
  ...
  public function teacher_access($course_id = ""){
    if($course_id != ""){
      ...
      if(session()->get('teacher_login') == 1 && $course_data['user_id'] != session()->get('user_id')){
        session()->setFlashdata('error_message', get_phrase('you_do_not_have_access_to_this_course'));
        return redirect()->to(site_url('addons/courses'));
      }
    }
  }
  ...
  public function course_preview($course_id = ""){
    $page_data['course'] = $this->lms_model->get_course_by_id($course_id);
    return view('backend/academy/course_preview', $page_data);
  }
  public function course_information($course_id = ""){
    $page_data['course'] = $this->lms_model->get_course_by_id($course_id);
    return view('backend/academy/course_information', $page_data);
  }

  // Manage Exam Questions
  public function exam_questions($exam_id = "", $action = "", $question_id = "") {
    $this->student_access_denied();
    $exam_details = $this->toRowArray($this->lms_model->get_exams('exam', $exam_id));
...
public function save_exam_questions() {
    ...
    // Verify exam exists
    $exam = $this->toRowArray($this->lms_model->get_exams('exam', $exam_id));
    if (!$exam) {
        echo json_encode(array(
            'success' => false,
    ...
    if (!empty($questions_json)) {
        ...
        if (is_array($questions)) {
            ...
            $this->lms_model->delete_exam_questions_by_exam($exam_id);
            ...
            foreach ($questions as $q) {
                ...
                $this->lms_model->insert_exam_question($question_data);
            }
        }
    }

    // Get updated question count
    $question_count = $this->lms_model->count_exam_questions($exam_id);
...
public function get_exam_questions_json($exam_id = "") {
    ...
    $questions_result = $this->toArrayResult($this->lms_model->get_exam_questions($exam_id));
...
public function get_exam_question($question_id = "") {
    ...
    $question = $this->toRowArray($this->lms_model->get_exam_question_by_id($question_id));
...
public function generate_questions_from_pdf()
  {
    ...
    $generation_key = 'pdf_generation_in_progress_' . session()->get('user_id');
    $generation_lock = session()->get($generation_key);
    ...
    session()->set($generation_key, time());

    $exam_id         = (int) ($this->request->getPost('exam_id') ?? 0);
    $num_questions   = (int) $this->request->getPost('number_of_questions');
    $difficulty_level = esc($this->request->getPost('difficulty_level')) ?? 'medium';
    $overwrite_flag  = strtolower((string) $this->request->getPost('overwrite_existing'));
    ...
    if ($exam_id <= 0) {
      session()->remove($generation_key);
      $this->respond_json([
        'status' => false,
        'message' => 'Exam ID invalide'
      ], 400);
      return;
    }
    ...
    $newName = $pdf_file->getRandomName();
    $pdf_file->move($upload_path, $newName);
    $pdf_path = $upload_path . $newName;
    ...
    if ($added === 0) {
      $this->db->transComplete(false);
      session()->remove($generation_key);
      $this->respond_json(array(
        'status'  => false,
        'message' => 'Aucune nouvelle question n\'a pu être créée. Vérifiez le format des questions générées ou le contenu du PDF.',
        'details' => array(
          'generated_count' => is_array($questions) ? count($questions) : 0,
    ...
    $this->db->transComplete(true);

    // Libérer le verrouillage après succès
    session()->remove($generation_key);

    // Get total question count for this exam
    $total_questions = $this->lms_model->count_exam_questions($exam_id);
    ...
    try {
      ...
      if (preg_match('/\[[\s\S]*\]/', $content, $m)) {
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
          ...
          if (is_array($json)) {
            // If AI returned a single question object, normalize it to an array.
            if (isset($json['title']) && isset($json['options'])) {
              return [$json];
            }
            return $json;
          }
  ...
  {
    $exam_id = (int) $exam_id;
    if ($exam_id <= 0 || !is_array($question_data)) {
      return false;
    }

    // Validation minimale
    if (empty($question_data['title']) || empty($question_data['options'])) {
      return false;
    }
    ...
    $correct_answer_raw = $question_data['correct_answer'] ?? ($question_data['correct_answers'] ?? null);

    if (!is_array($options)) {
      return false;
    }
    // Convert associative options (A/B/C/D => "...") to indexed array.
    ...
    foreach ($options as $opt) {
      ...
      if ($clean === '') {
        $clean = 'Option ' . (count($clean_options) + 1);
      }
      $clean_options[] = $clean;
    }

    // Correct answer normalization: accepts 1..4, 0..3, A..D, or option text.
    $correct_answer = 1;
    if (is_array($correct_answer_raw)) {
      $correct_answer_raw = $correct_answer_raw[0] ?? null;
    }
    if (is_numeric($correct_answer_raw)) {
      $numeric = (int) $correct_answer_raw;
      if ($numeric >= 1 && $numeric <= 4) {
        $correct_answer = $numeric;
      } elseif ($numeric >= 0 && $numeric <= 3) {
        $correct_answer = $numeric + 1;
      }
    } elseif (is_string($correct_answer_raw)) {
      $token = strtoupper(trim($correct_answer_raw));
      $letterMap = ['A' => 1, 'B' => 2, 'C' => 3, 'D' => 4];
      if (isset($letterMap[$token])) {
        $correct_answer = $letterMap[$token];
      } else {
        $idx = array_search($this->sanitize_multilingual_text($correct_answer_raw), $clean_options, true);
        if ($idx !== false) {
          $correct_answer = ((int) $idx) + 1;
        }
      }
    }
    ...
    $data = [
      ...
      'order'             => $this->lms_model->count_all_questions('exam_questions') + 1 // ordre auto
    ];

    return $this->lms_model->insert_exam_question($data);
  }
  ...
  public function extract_pdf_structure()
  {
    ...
    if (!session()->get('teacher_login') && 
        !session()->get('admin_login') && 
        !session()->get('superadmin_login')) {
    ...
    // Run Python extraction script (support both CI3-style and CI4-style locations)
    $possible_scripts = [
      ROOTPATH . 'app/scripts/extract_pdf_structure.py',
      FCPATH . 'application/scripts/extract_pdf_structure.py',
      ROOTPATH . 'application/scripts/extract_pdf_structure.py',
    ];
    $script = null;
    foreach ($possible_scripts as $candidate_script) {
    ...
    if (!$script) {
      unlink($temp_path);
      log_message('error', 'Extraction script not found. Checked: ' . implode(', ', $possible_scripts));
...
/**
  ...
  public function generate_outline_schemas()
  {
    ...
    // Get POST data
    $course_id = $this->request->getPost('course_id') ?? 0;
    $pdf_json_path = $this->request->getPost('pdf_json_path') ?? '';
    $outline_rules_json = $this->request->getPost('outline_rules') ?? '{}';
    ...
    $course_context = [];
    if ($course_id > 0) {
      $course = $this->db_helper_model->get_by_id('course', $course_id);
      if ($course) {
        $course_context = [
...
PROMPT;
  ...
  {
    $api_key = $this->deepseekApiKey !== '' ? $this->deepseekApiKey : (string) (function_exists('env') ? env('ai.deepseekApiKey', '') : '');
    if ($api_key === '') {
  ...
  public function apply_outline_schema()
  {
    ...
    $course_id = $this->request->getPost('course_id');
    $schema_json = $this->request->getPost('schema');
    $outline_rules_json = $this->request->getPost('outline_rules') ?? '{}';
    $outline_rules = json_decode($outline_rules_json, true) ?? [];

    if (empty($course_id) || empty($schema_json)) {
      return $this->respond_json(['error' => 'Missing required parameters'], 400);
    }
    
    // Verify course exists and user has access
    $course = $this->lms_model->get_course_by_id($course_id);
    if (!$course) {
      return $this->respond_json(['error' => 'Course not found'], 404);
    }
    ...
    if (!$schema) {
      return $this->respond_json(['error' => 'Invalid schema format'], 400);
    }
    ...
    if (empty($sections)) {
      return $this->respond_json(['error' => 'No sections found in schema'], 400);
    }
    ...
    $section_order = $this->lms_model->get_max_section_order($course_id);
    ...
    foreach ($sections as $section_data) {
      ...
      $section_id = $this->lms_model->insert_course_section($section_insert);
      $created_sections++;
      ...
      foreach ($children as $child) {
        ...
        if ($child_type == 'quiz' && ($outline_rules['quizFrequency'] ?? 'per_section') !== 'none') {
          ...
          $quiz_id = $this->lms_model->insert_lesson($quiz_data);
          $created_quizzes++;
          ...
          $lesson_id = $this->lms_model->insert_lesson($lesson_data);
          $created_lessons++;
        }
      }
    }
    ...
    $this->lms_model->update_course($course_id, ['section' => json_encode($previous_sections)]);
  ...
  public function generate_quiz_from_pdf()
  {
    ...
    // Anti-duplicate lock
    $generation_key = 'quiz_pdf_generation_' . session()->get('user_id');
    $generation_lock = session()->get($generation_key);
    ...
    $questions_count = (int) esc($this->request->getPost('questions_count')) ?? 10;
    ...
    $max_order = $this->lms_model->get_max_lesson_order($section_id);
    ...
    foreach ($questions as $q) {
      ...
      $question_id = $this->lms_model->insert_question($question_data);
      if ($question_id) {
        $added_questions++;
      }
    }
  ...
  public function get_lessons_for_quiz($course_id = '', $target_section_id = '')
  {
    ...
    $sections_to_include = $this->request->getPost('sections_to_include');
    if (!empty($sections_to_include)) {
      ...
      // Get sections by IDs
      $sections = $this->lms_model->get_sections_by_ids($course_id, $section_ids);
    } else {
      // Legacy behavior: Get all sections before the target section (orders < target_order)
      $target_section = $this->lms_model->get_section_by_id($target_section_id);
      ...
      $sections = $this->lms_model->get_sections_before_order($course_id, $target_order);
    }
    ...
    foreach ($sections as $section) {
      ...
      $lessons = $this->lms_model->get_text_lessons_by_section($section['id']);
  ...
  public function generate_quiz_from_lessons()
  {
    ...
    // Anti-duplicate lock
    $generation_key = 'quiz_lessons_generation_' . session()->get('user_id');
    $generation_lock = session()->get($generation_key);
    ...
    $questions_count = (int) esc($this->request->getPost('questions_count')) ?? 10;
    $difficulty = $this->request->getPost('difficulty') ?: 'medium';
    $lesson_ids_json = $this->request->getPost('lesson_ids');
    ...
    foreach ($lesson_ids as $lesson_id) {
      $lesson = $this->lms_model->get_lesson_title_summary($lesson_id);
  ...
  {
    return 'lesson_ai_generation_' . session()->get('user_id');
  }
  ...
  private function _validate_ai_lesson_request()
  {
    ...
    // Ensure lock is released even if an unexpected fatal error occurs.
  ...
  private function _validate_ai_lesson_input_data($data)
  {
    ...
    if ($data['source'] === 'pdf') {
      ...
      $isValidPdf = $pdfFile && method_exists($pdfFile, 'isValid') && $pdfFile->isValid();
      if (!$isValidPdf) {
        return [
          'success' => false,
  ...
  private function _process_ai_lesson_generation($input_data)
  {
    try {
      ...
      $section = $this->toRowArray($this->lms_model->get_section('section', $input_data['section_id']));
      ...
      if ($input_data['source'] === 'pdf') {
        $pdfFile = $input_data['pdf_file'] ?? null;
        $isValidPdf = $pdfFile && method_exists($pdfFile, 'isValid') && $pdfFile->isValid();
        if (!$isValidPdf) {
          throw new Exception(get_phrase('pdf_file_required'));
        }

        $uploadPath = FCPATH . 'uploads/ai_pdfs/';
        if (!is_dir($uploadPath)) {
          mkdir($uploadPath, 0755, true);
        }

        $extension = strtolower((string) $pdfFile->getClientExtension());
        if ($extension !== 'pdf') {
          throw new Exception(get_phrase('invalid_file_type'));
        }

        // Keep CI3 behavior: max size constant is in KB.
        $maxBytes = self::PDF_MAX_SIZE_BYTES * 1024;
        if ($pdfFile->getSize() > $maxBytes) {
          throw new Exception(get_phrase('file_too_large'));
        }
        ...
        $context = $this->extract_text_from_pdf($pdfPath);
        ...
        // Use course outline
        $sections = $this->toArrayResult($this->lms_model->get_section('course', $input_data['course_id']));
        $course_outline = "Course: " . $course['title'] . "\n\n";

        foreach ($sections as $sec) {
          $course_outline .= "Section: " . $sec['title'] . "\n";
          $lessons = $this->toArrayResult($this->lms_model->get_lessons('section', $sec['id']));
          foreach ($lessons as $lesson) {
      ...
      $current_lessons = $this->toArrayResult($this->lms_model->get_lessons('section', $input_data['section_id']));
  ...
  private function _process_ai_lesson_update($input_data)
  {
    try {
      ...
      $course = $this->lms_model->get_course_by_id($input_data['course_id']);
      $section = $this->toRowArray($this->lms_model->get_section('section', $input_data['section_id']));
      $existing_lesson = $this->toRowArray($this->lms_model->get_lessons('lesson', $input_data['lesson_id']));
      ...
      $this->lms_model->update_lesson($input_data['lesson_id'], $lesson_data);
...
PROMPT;
  ...
  private function toRowArray($result): array
  {
    if (is_array($result)) {
      // Already a row array OR an array of rows
    ...
    if (is_object($result)) {
      if (method_exists($result, 'row_array')) {
        $row = $result;
        return is_array($row) ? $row : [];
      }
      if (method_exists($result, 'getRowArray')) {
        $row = $result->getRowArray();
        return is_array($row) ? $row : [];
      }
      if (method_exists($result, 'getRow')) {
        $row = $result->getRow();
        if (is_array($row)) {
          return $row;
        }
      ...
      if (method_exists($result, 'getResult')) {
        ...
        if (is_array($rows) && !empty($rows)) {
          $first = $rows[0];
          return is_object($first) ? (array) $first : (is_array($first) ? $first : []);
        }
      }
      return (array) $result;
    }
